...
TARGET_DIR="$REPO_ROOT/target/examples/typescript/minipi"
WIT_DIR="$REPO_ROOT/wit/plexspaces-actor"
OUTPUT_WASM="$TARGET_DIR/minipi_actor.wasm"
ESBUILD_STAMP="$SCRIPT_DIR/node_modules/.esbuild-platform"

mkdir -p "$TARGET_DIR"
cd "$SCRIPT_DIR"

echo "Building MiniPi Actors (TypeScript WASM)..."
...
if [ -d "$REPO_ROOT/sdks/typescript" ]; then
  ...
  # Sync freshly-built SDK dist into local node_modules so tsc sees all exports
...
./node_modules/.bin/jco componentize minipi_actors_bundle.mjs --wit "$WIT_DIR" -n actor-world -o "$OUTPUT_WASM" --disable all
