#!/bin/bash
# Build Storefront API WASM actor (store config + cart + checkout rate limit via host KV)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
SDK_DIR="$PROJECT_ROOT/sdks/python"
ACTOR_NAME="storefront_actor"
...
echo "Building $ACTOR_NAME (Storefront API: config + cart + checkout limit)..."
...
echo "Built: ${ACTOR_NAME}.wasm ($(ls -lh ${ACTOR_NAME}.wasm 2>/dev/null | awk '{print $5}' || echo '?'))"
