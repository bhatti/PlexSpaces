...
echo "cd $(pwd)"
echo "NODE_ID=node1 NODE_ADDRESS=localhost:8000 SPAWN_SERVICES=orders,payment cargo run --bin order-coordinator"
echo ""
echo -e "${YELLOW}Terminal 2 - Node 2 (Inventory):${NC}"
echo "cd $(pwd)"
echo "NODE_ID=node2 NODE_ADDRESS=localhost:8001 SPAWN_SERVICES=inventory cargo run --bin order-coordinator"
echo ""
echo -e "${YELLOW}Terminal 3 - Node 3 (Shipping):${NC}"
echo "cd $(pwd)"
echo "NODE_ID=node3 NODE_ADDRESS=localhost:8002 SPAWN_SERVICES=shipping cargo run --bin order-coordinator"
echo ""
