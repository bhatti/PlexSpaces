...
use plexspaces_core::{BehaviorRegistry, Actor, Message, BehaviorFactory};
...
async fn test_embedded_application_explicit_behavior_registration() {
    ...
    assert!(registry_opt.is_some(), "BehaviorRegistry should be registered");
    
    let registry = registry_opt.unwrap();
    assert!(registry.is_registered("worker").await, "worker behavior should be registered");
    assert!(!registry.is_registered("unknown").await, "unknown behavior should not be registered");
    
    // Verify behavior can be created
    let behavior_result = registry.create("worker", &[]).await;
    assert!(behavior_result.is_ok(), "Should be able to create worker behavior");
}
...
async fn test_behavior_registry_arc_interior_mutability() {
    ...
    registry.register_simple("actor1", || {
        Box::pin(async move {
    ...
    registry.register_simple("actor2", || {
        Box::pin(async move {
            Ok(Box::new(TestActor {
