...
use plexspaces_core::{
    ActorId, MessageSender, ServiceLocator as ServiceLocatorTrait, VirtualActorManager,
};
...
impl MessageSender for VirtualActorWrapper {
    async fn tell(&self, message: Message) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if registry.lookup_actor(&self.actor_id).await.is_none() {
            tracing::warn!(
                "[VIRTUAL_ACTOR_WRAPPER] Actor not registered: actor_id={}",
                self.actor_id
            );
            return Err(format!(
        ...
        if tracing::enabled!(tracing::Level::DEBUG) {
            tracing::debug!(
                "[VIRTUAL_ACTOR_WRAPPER] Actor active check: actor_id={}, is_active={}",
                self.actor_id,
                is_active
            );
        }
        if !is_active {
            ...
            if tracing::enabled!(tracing::Level::DEBUG) {
                tracing::debug!(
                    "[VIRTUAL_ACTOR_WRAPPER] Calling activate_virtual_actor (SYNC): actor_id={}",
                    self.actor_id
                );
            }
            ...
            if !is_active_after {
                tracing::warn!(
                    "[VIRTUAL_ACTOR_WRAPPER] Actor not active after activation: actor_id={}",
                    self.actor_id
                );
                return Err(format!(
                    "Actor {} is not active after synchronous activation",
                    self.actor_id
                )
                .into());
            }
        ...
        match &result {
            Ok(_) => {
                if tracing::enabled!(tracing::Level::DEBUG) {
                    tracing::debug!(
                        "[VIRTUAL_ACTOR_WRAPPER] Successfully sent message via MessageSender"
                    );
                }
            }
            Err(e) => {
                tracing::warn!(
                    "[VIRTUAL_ACTOR_WRAPPER] Failed to send message via MessageSender: {}",
                    e
                );
            }
        }
