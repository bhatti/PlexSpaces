...
//! store.put("key", b"value".to_vec()).await?;
...
impl KeyValueStore for RedisKVStore {
    ...
    async fn list(&self, prefix: &str) -> KVResult<Vec<String>> {
        ...
        Ok(keys.into_iter().map(|k| self.unprefixed_key(&k)).collect())
    }
    ...
    async fn multi_put(&self, entries: &[(&str, Vec<u8>)]) -> KVResult<()> {
        let mut conn = self.manager.clone();

        // Use pipeline for efficiency
        let mut pipe = redis::pipe();
        for (key, value) in entries {
            pipe.set(self.prefixed_key(key), value.clone());
        }
    ...
    async fn cas(&self, key: &str, expected: Option<Vec<u8>>, new: Vec<u8>) -> KVResult<bool> {
        let mut conn = self.manager.clone();
        let prefixed = self.prefixed_key(key);
    ...
    async fn watch(&self, _key: &str) -> KVResult<mpsc::Receiver<KVEvent>> {
    ...
    async fn watch_prefix(&self, _prefix: &str) -> KVResult<mpsc::Receiver<KVEvent>> {
        // TODO: Implement Redis keyspace notifications
        // Requires CONFIG SET notify-keyspace-events and separate connection
        // For now, return empty channel (not implemented yet)
        let (_tx, rx) = mpsc::channel(100);
...
mod tests {
    ...
    async fn test_ttl_operations() {
        ...
        store
            .put_with_ttl("ttl_key", b"value".to_vec(), Duration::from_secs(60))
            .await
            .unwrap();

        let ttl = store.get_ttl("ttl_key").await.unwrap();
        assert!(ttl.is_some());
    ...
    async fn test_atomic_increment() {
        ...
        let v1 = store.increment("counter", 5).await.unwrap();
        let v2 = store.increment("counter", 3).await.unwrap();
    ...
    async fn test_cas_success() {
        ...
        store.put("cas_key", b"old".to_vec()).await.unwrap();

        let success = store
            .cas("cas_key", Some(b"old".to_vec()), b"new".to_vec())
            .await
            .unwrap();

        assert!(success);

        let value = store.get("cas_key").await.unwrap();
        assert_eq!(value, Some(b"new".to_vec()));

        // Cleanup
        store.delete("cas_key").await.unwrap();
    }
    ...
    async fn test_list_prefix() {
        ...
        store.put("prefix:key1", b"v1".to_vec()).await.unwrap();
        store.put("prefix:key2", b"v2".to_vec()).await.unwrap();
        store.put("other:key", b"v3".to_vec()).await.unwrap();

        let keys = store.list("prefix:").await.unwrap();
        assert_eq!(keys.len(), 2);

        // Cleanup
        store.clear_prefix("prefix:").await.unwrap();
        store.delete("other:key").await.unwrap();
    }
}
