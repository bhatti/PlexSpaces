...
fn with_state<T>(f: impl FnOnce(&mut PipelineState) -> T) -> T {
    let mut g = state_cell()
        .lock()
        .expect("entity_recognition state lock poisoned");
    f(&mut *g)
}
...
fn handle_get_metrics() -> String {
    ...
    match application_get_metrics(&application_id, &node_id) {
...
impl EntityRecognitionActor {
    ...
    fn process_document_op(
        ...
        let payload = parse_payload(payload)?;
        Ok(json_bytes(
            serde_json::from_str(&handle_process_document(&payload)).unwrap_or_else(
                |_| serde_json::json!({ "error": "invalid process_document response" }),
            ),
        ))
    }
...
impl Guest for EntityRecognitionBridge {
    ...
    fn set_state(state: Vec<u8>) -> Result<(), String> {
        ...
        match serde_json::from_slice::<PipelineState>(&state) {
            Ok(s) => {
