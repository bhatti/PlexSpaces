...
TARGET_DIR="$REPO_ROOT/target/examples/python/web_crawl"
...
TEMP_CONFIG="$(mktemp -t py-web-crawl-config)"
...
echo "  Testing orchestrator status (pool_utilization, worker_stats)..."
...
for r in results:
    ...
    print(f"  │ {w:>7} │ {pages:>6} │ {elapsed:>7} ms │ {coord:>8} │ {fetch:>8} │ {float(pps):>11.1f} │ {float(speedup):>7.2f}x │ {float(eff):>7.1f}% │ {float(pf):>11.1%} │ {tw:>5}w/{uw:>3}u │")
print("  └─────────┴────────┴───────────┴──────────┴──────────┴─────────────┴─────────┴──────────┴──────────────┴─────────────┘")
print()
...
echo ""
echo "================================================================"
echo -e "  ${GREEN}Web Crawl (Python WASM) Test Complete${NC}"
echo ""
echo "  Key behaviors demonstrated:"
echo "    ✓ TupleSpace frontier — url_queue as live work frontier, ts.take() atomic claim"
echo "    ✓ ElasticPool         — pool_checkout/checkin, utilization metrics"
