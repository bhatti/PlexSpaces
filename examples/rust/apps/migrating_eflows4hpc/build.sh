#!/usr/bin/env bash
# Build EFlows4HPC Ensemble WASM component (Rust): core wasm → component embed → WASI adapter
set -euo pipefail
...
EMBEDDED_WASM="$SCRIPT_DIR/ensemble_actor_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/migrating_eflows4hpc_wasm.wasm"
...
echo "Building EFlows4HPC Ensemble (Rust WASM, SDK+WIT)..."
