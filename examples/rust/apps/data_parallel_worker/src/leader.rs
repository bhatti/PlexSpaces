...
pub(super) fn handle_leader_run(payload: &[u8]) -> Vec<u8> {
    ...
    for node_id in &participant_node_ids {
        ...
        match application_status(node_id) {
            ...
            Err(err) => {
                return super::json_bytes(serde_json::json!({
                    "error": format!("failed to collect application status for {}: {}", node_id, err),
                }));
            }
        }
    }
    ...
    for node_id in &participant_node_ids {
        let Some(start_metrics_for_node) = start_metrics.get(node_id) else {
            continue;
        };
        let Some(end_metrics_for_node) = end_metrics.get(node_id) else {
            continue;
        };
        ...
        accumulate_metrics_delta(
            ...
            start_metrics_for_node,
            end_metrics_for_node,
        );
    }
