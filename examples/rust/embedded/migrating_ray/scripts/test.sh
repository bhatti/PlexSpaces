#!/bin/bash
# Test script for Ray comparison example
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Ray Comparison Example - Test Suite                          ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
BUILD_OUTPUT=$(cargo build --release 2>&1 | tee /tmp/ray_build.log) || BUILD_EXIT=$?
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
if timeout 60 cargo run --release 2>&1 | tee /tmp/ray_output.log; then
...
# Check for ray/workflow output
if grep -qi "ray\|parameter.*server\|worker\|comparison" /tmp/ray_output.log; then
    echo -e "${GREEN}✅ Found parameter server execution output${NC}"
else
    echo -e "${RED}❌ Missing parameter server execution output${NC}"
    VALIDATION_PASSED=false
fi

# Check for completion
if grep -qi "completed\|successful\|training summary" /tmp/ray_output.log; then
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
if grep -q "Total Time:" /tmp/ray_output.log; then
    grep "Total Time:" /tmp/ray_output.log | tail -1
fi
if grep -q "Iterations:" /tmp/ray_output.log; then
    grep "Iterations:" /tmp/ray_output.log | tail -1
fi
if grep -q "Workers:" /tmp/ray_output.log; then
    grep "Workers:" /tmp/ray_output.log | tail -1
fi
if grep -q "Avg Time/Iter:" /tmp/ray_output.log; then
    grep "Avg Time/Iter:" /tmp/ray_output.log | tail -1
fi
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
