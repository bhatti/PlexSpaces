#!/bin/bash
# Test Order Fulfillment Workflow - Python WASM
...
APP_ID="temporal-order-fulfillment-py"
...
[ ! -f "$WASM_FILE" ] && { echo "Building..."; "$SCRIPT_DIR/build.sh" || exit 1; }

HTTP_CHECK=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:$HTTP_PORT/" 2>/dev/null) || HTTP_CHECK="000"
[ "$HTTP_CHECK" = "000" ] && { echo -e "${RED}Start node: ./scripts/server.sh${NC}"; exit 1; }
...
echo "$RUN" | grep -q '"status":"shipped"' && echo -e "  ${GREEN}Order order-1 shipped${NC}" || echo "  $RUN"

echo "Step 3: Query status"
send_op "order-1" '{"op":"workflow_query:status"}' 5 | head -c 100
echo "..."
...
send_op "order-2" '{"op":"workflow_query:status"}' 5 | grep -q '"cancel_requested":true' && echo -e "  ${GREEN}Cancel received${NC}"
...
echo ""
echo "================================================================"
echo -e "  ${GREEN}Order Fulfillment Workflow (Python) Test Complete${NC}"
echo "================================================================"
