...
use plexspaces_actor::{ChildType, RestartPolicy, SupervisionStrategy, Supervisor};
use std::sync::Arc;
...
impl SupervisorBuilder {
    ...
    ///
    /// ## Errors
    /// - `ApplicationError::ConfigError` if spec is invalid
    ///
    /// ## Design Notes
    ...
    fn convert_supervision_strategy(
        ...
        match ProtoSupervisionStrategy::try_from(spec.strategy) {
            ...
            Err(_) => Err(ApplicationError::ConfigError(format!(
...
mod tests {
    ...
    async fn test_convert_supervision_strategy_one_for_one() {
        ...
        match strategy.unwrap() {
            plexspaces_actor::SupervisionStrategy::OneForOne {
                max_restarts,
                within_seconds,
            } => {
    ...
    async fn test_convert_supervision_strategy_one_for_all() {
        ...
        match strategy.unwrap() {
            plexspaces_actor::SupervisionStrategy::OneForAll {
                max_restarts,
                within_seconds,
            } => {
    ...
    async fn test_convert_supervision_strategy_rest_for_one() {
        ...
        match strategy.unwrap() {
            plexspaces_actor::SupervisionStrategy::RestForOne {
                max_restarts,
                within_seconds,
            } => {
    ...
    async fn test_convert_restart_policy_permanent() {
        ...
        assert!(matches!(
            policy.unwrap(),
            plexspaces_actor::RestartPolicy::Permanent
        ));
    }
    ...
    async fn test_convert_restart_policy_transient() {
        ...
        assert!(matches!(
            policy.unwrap(),
            plexspaces_actor::RestartPolicy::Transient
        ));
    }
    ...
    async fn test_convert_restart_policy_temporary() {
        ...
        assert!(matches!(
            policy.unwrap(),
            plexspaces_actor::RestartPolicy::Temporary
        ));
    }
    ...
    async fn test_convert_child_type_worker() {
        ...
        assert!(matches!(
            child_type.unwrap(),
            plexspaces_actor::ChildType::Worker
        ));
    }
    ...
    async fn test_convert_child_type_supervisor() {
        ...
        assert!(matches!(
            child_type.unwrap(),
            plexspaces_actor::ChildType::Supervisor
        ));
    }
    ...
    async fn test_validate_child_spec_empty_id() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            ApplicationError::ConfigError(_)
        ));
    }
