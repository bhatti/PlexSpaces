...
//! Memory lock manager integration tests.
//!
//! These tests verify:
...
mod tests {
    ...
    /// Create a new memory lock manager
    fn create_manager() -> MemoryLockManager {
        MemoryLockManager::new()
    }

    #[tokio::test]
    async fn test_memory_acquire_lock() {
    ...
    #[tokio::test]
    async fn test_memory_acquire_lock_already_held() {
        let manager = create_manager();
    ...
    #[tokio::test]
    async fn test_memory_acquire_lock_same_holder() {
        let manager = create_manager();
        ...
        // Same holder acquiring again should be reentrant and return the current lock.
        let lock2 = manager
            .acquire_lock(
                &test_ctx(),
                AcquireLockOptions {
    ...
    #[tokio::test]
    async fn test_memory_renew_lock_same_holder_allows_stale_version() {
        let manager = create_manager();
        ...
        assert_ne!(renewed_once.version, renewed_twice.version);
        assert_eq!(renewed_twice.lease_duration_secs, 90);
        ...
        let retrieved = manager
            ...
            .unwrap()
            .unwrap();
        assert_eq!(retrieved.version, renewed_twice.version);
        assert_eq!(retrieved.lease_duration_secs, 90);
    }

    #[tokio::test]
    async fn test_memory_renew_lock_wrong_holder() {
        let manager = create_manager();
    ...
    #[tokio::test]
    async fn test_memory_release_lock() {
        let manager = create_manager();
    ...
    #[tokio::test]
    async fn test_memory_release_lock_without_delete() {
        let manager = create_manager();
    ...
    #[tokio::test]
    async fn test_memory_acquire_expired_lock() {
        let manager = create_manager();
    ...
    #[tokio::test]
    async fn test_memory_lock_metadata() {
        let manager = create_manager();
    ...
    #[tokio::test]
    async fn test_memory_concurrent_lock_acquisition() {
        ...
        // Verify in memory
        let retrieved = manager
            .get_lock(&test_ctx(), "concurrent-lock")
            .await
            .unwrap()
            .unwrap();
    ...
    #[tokio::test]
    async fn test_memory_multiple_locks() {
        let manager = create_manager();
    ...
    #[tokio::test]
    async fn test_memory_tenant_isolation() {
        let manager = create_manager();
        let ctx1 =
    ...
    async fn test_memory_namespace_isolation() {
        ...
        let ctx1 =
            RequestContext::new_without_auth("tenant-1".to_string(), "namespace-1".to_string());
        let ctx2 =
            RequestContext::new_without_auth("tenant-1".to_string(), "namespace-2".to_string());
        let lock_key = "same-lock-key".to_string();
