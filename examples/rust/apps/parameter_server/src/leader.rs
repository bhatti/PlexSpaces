...
pub(super) fn handle_leader_run(payload_json: &str) -> String {
    ...
    if request.worker_count == 0 || request.iterations == 0 {
        return serde_json::json!({ "error": "worker_count and iterations must be greater than zero" })
            .to_string();
    }

    let group_id = format!("parameter-server-{}", host::now_ms());
        ...
        for shard in shard_responses {
            ...
            if let Some(error_message) = payload.get("error").and_then(|value| value.as_str()) {
                ...
                host::log(
                    "info",
                    &format!(
                        "parameter_server shard error actor_id={} error={}",
