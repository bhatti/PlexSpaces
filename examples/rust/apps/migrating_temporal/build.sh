...
EMBEDDED_WASM="$SCRIPT_DIR/order_fulfillment_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/migrating_temporal_wasm.wasm"
...
if ! rustup target list --installed | grep -qx 'wasm32-wasip1'; then
  echo "ERROR: rustup target add wasm32-wasip1"
  exit 1
fi

echo "Building order fulfillment workflow (Rust WASM, SDK+WIT)..."
