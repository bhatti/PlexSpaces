...
impl WorkerActor {
    ...
    fn process_shard_work(
        &mut self,
        _from_actor: &str,
        payload_json: &str,
    ) -> Result<String, String> {
        Ok(handle_worker_process_shard_work(payload_json))
    }
}
...
pub(super) fn handle_worker_process_shard_work(payload_json: &str) -> String {
    let request: GradientRequest = match serde_json::from_str(payload_json) {
        Ok(request) => request,
        Err(err) => {
            return serde_json::json!({ "error": format!("invalid process_shard_work payload: {}", err) })
                .to_string()
        }
    };
