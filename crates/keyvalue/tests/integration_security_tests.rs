...
mod security_tests {
    ...
    fn malicious_ctx() -> RequestContext {
        RequestContext::new_without_auth(
            "tenant1'; DROP TABLE kv_store; --".to_string(),
            "default".to_string(),
        )
    }
    ...
    async fn test_unauthorized_tenant_cannot_access_other_tenant_data() {
        ...
        kv.put(&ctx1, "secret:key", b"tenant1-secret".to_vec())
            .await
            .unwrap();
        kv.put(&ctx1, "config:api_key", b"tenant1-api-key".to_vec())
            .await
            .unwrap();

        // Tenant2 stores their own data
        kv.put(&ctx2, "secret:key", b"tenant2-secret".to_vec())
            .await
            .unwrap();
        ...
        kv.put(&ctx3, "secret:key", b"tenant3-secret".to_vec())
            .await
            .unwrap();
        assert_eq!(
            kv.get(&ctx3, "secret:key").await.unwrap(),
            Some(b"tenant3-secret".to_vec())
        );

        // Verify tenant1's data is still isolated
        assert_eq!(
            kv.get(&ctx1, "secret:key").await.unwrap(),
            Some(b"tenant1-secret".to_vec())
        );
        assert_eq!(
            kv.get(&ctx2, "secret:key").await.unwrap(),
            Some(b"tenant2-secret".to_vec())
        );
    }
    ...
    async fn test_unauthenticated_request_cannot_access_any_tenant_data() {
        ...
        // Tenant1 stores data
        kv.put(&ctx1, "user:alice", b"alice-data".to_vec())
            .await
            .unwrap();
        kv.put(&ctx1, "user:bob", b"bob-data".to_vec())
            .await
            .unwrap();
        ...
        assert_eq!(
            keys.len(),
            0,
            "Unauthenticated request should not see tenant1's data"
        );

        // Unauthenticated can only see their own empty tenant namespace
        kv.put(&unauthenticated, "temp:data", b"temp".to_vec())
            .await
            .unwrap();
    ...
    async fn test_tenant_isolation_enforced_at_storage_layer() {
        ...
        for i in 0..100 {
            ...
            kv.put(&ctx1, &key, format!("tenant1-value-{}", i).into_bytes())
                .await
                .unwrap();
            kv.put(&ctx2, &key, format!("tenant2-value-{}", i).into_bytes())
                .await
                .unwrap();
        }
        ...
        for i in 0..100 {
            ...
            assert!(
                value_str.starts_with("tenant1-value-"),
                "Tenant1 should only see their own data, got: {}",
                value_str
            );
        }
        ...
        for i in 0..100 {
            ...
            let value_str = String::from_utf8(value).unwrap();
            assert!(
                value_str.starts_with("tenant2-value-"),
                "Tenant2 should only see their own data, got: {}",
                value_str
            );
        }
    }
    ...
    async fn test_sql_injection_attempt_via_tenant_id_is_isolated() {
        ...
        // Tenant1 stores data
        kv.put(&ctx1, "data:1", b"tenant1-data".to_vec())
            .await
            .unwrap();
        ...
        kv.put(&malicious, "data:1", b"malicious-data".to_vec())
            .await
            .unwrap();
        assert_eq!(
            kv.get(&malicious, "data:1").await.unwrap(),
            Some(b"malicious-data".to_vec())
        );

        // Tenant1's data should still be safe
        assert_eq!(
            kv.get(&ctx1, "data:1").await.unwrap(),
            Some(b"tenant1-data".to_vec())
        );
    }
    ...
    async fn test_list_operations_respect_tenant_isolation() {
        ...
        for i in 0..50 {
            ...
            kv.put(&ctx1, &key, format!("tenant1-actor-{}", i).into_bytes())
                .await
                .unwrap();
        }
        ...
        for i in 0..30 {
            let key = format!("actor:{}", i);
            kv.put(&ctx2, &key, format!("tenant2-actor-{}", i).into_bytes())
                .await
                .unwrap();
        }
        ...
        assert_eq!(
            keys3.len(),
            0,
            "Unauthorized tenant should not see any keys"
        );
    ...
    async fn test_delete_operations_respect_tenant_isolation() {
        ...
        kv.put(&ctx1, "shared:key", b"tenant1-value".to_vec())
            .await
            .unwrap();
        kv.put(&ctx2, "shared:key", b"tenant2-value".to_vec())
            .await
            .unwrap();
        ...
        assert_eq!(
            kv.get(&ctx1, "shared:key").await.unwrap(),
            Some(b"tenant1-value".to_vec())
        );
        assert_eq!(
            kv.get(&ctx2, "shared:key").await.unwrap(),
            Some(b"tenant2-value".to_vec())
        );
    ...
    async fn test_stats_respect_tenant_isolation() {
        ...
        for i in 0..100 {
            kv.put(&ctx1, &format!("key:{}", i), b"value".to_vec())
                .await
                .unwrap();
        }
        for i in 0..50 {
            kv.put(&ctx2, &format!("key:{}", i), b"value".to_vec())
                .await
                .unwrap();
        }
    ...
    async fn test_clear_prefix_respects_tenant_isolation() {
        ...
        // Store keys with same prefix in different tenants
        for i in 0..20 {
            kv.put(&ctx1, &format!("temp:{}", i), b"data".to_vec())
                .await
                .unwrap();
        }
        for i in 0..10 {
            kv.put(&ctx2, &format!("temp:{}", i), b"data".to_vec())
                .await
                .unwrap();
        }
    ...
    async fn test_cas_operations_respect_tenant_isolation() {
        ...
        let acquired1 = kv
            .cas(&ctx1, "lock:resource", None, b"tenant1-lock".to_vec())
            .await
            .unwrap();
        assert!(acquired1);
        ...
        let acquired2 = kv
            .cas(&ctx2, "lock:resource", None, b"tenant2-lock".to_vec())
            .await
            .unwrap();
        assert!(acquired2);
        ...
        let acquired3 = kv
            .cas(&ctx3, "lock:resource", None, b"tenant3-lock".to_vec())
            .await
            .unwrap();
        assert!(acquired3);

        // But each tenant should not be able to acquire again in their own tenant
        let acquired1_again = kv
            .cas(&ctx1, "lock:resource", None, b"tenant1-lock2".to_vec())
            .await
            .unwrap();
        ...
        assert_eq!(
            kv.get(&ctx1, "lock:resource").await.unwrap(),
            Some(b"tenant1-lock".to_vec())
        );
        assert_eq!(
            kv.get(&ctx2, "lock:resource").await.unwrap(),
            Some(b"tenant2-lock".to_vec())
        );
        assert_eq!(
            kv.get(&ctx3, "lock:resource").await.unwrap(),
            Some(b"tenant3-lock".to_vec())
        );
    }
