...
async fn test_health_check_overall_health() {
    ...
    assert!(
        status == ServingStatus::ServingStatusNotServing
            || status == ServingStatus::ServingStatusUnknown
    );
...
async fn test_health_check_service_specific() {
    ...
    let actor_service_status = health_reporter
        .get_service_status("plexspaces.actor.v1.ActorService")
        .await;
    ...
    let supervisor_status = health_reporter
        .get_service_status("plexspaces.supervisor.v1.SupervisorService")
        .await;
...
async fn test_health_check_shutdown() {
    ...
    // Begin shutdown
    health_reporter
        .begin_shutdown(Some(Duration::from_secs(1)))
        .await;
...
async fn test_health_check_liveness() {
    ...
    assert!(
        is_alive,
        "Node should be alive (not crashed) even during startup"
    );
...
async fn test_metrics_service_list_definitions() {
    ...
    let response = metrics_service
        .list_metric_definitions(request)
        .await
        .unwrap();
...
async fn test_health_status_propagation_on_shutdown() {
    ...
    for service_name in &services {
        ...
        assert_eq!(
            status,
            ServingStatus::ServingStatusNotServing,
            "Service {} should be NOT_SERVING after shutdown",
            service_name
        );
    }
}
...
async fn test_graceful_shutdown_drains_requests() {
    ...
    let (drained, duration, completed) = health_reporter
        .begin_shutdown(Some(Duration::from_millis(100)))
        .await;
