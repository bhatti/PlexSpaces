...
use plexspaces_proto::application::v1::{
    ApplicationSpec, SupervisionStrategy, ChildType, RestartPolicy,
};
...
pub fn save_wasm_app_atomically(
    ...
    if let Err(e) = fs::create_dir_all(&base_path) {
        return Err(format!("Failed to create apps directory {}: {}", base_path.display(), e));
    }

    // Create app subdirectory: wasm_apps_dir/app_name/
    let app_dir = base_path.join(app_name);
    if let Err(e) = fs::create_dir_all(&app_dir) {
        return Err(format!("Failed to create app directory {}: {}", app_dir.display(), e));
    }
    ...
    {
        let mut file = fs::File::create(&wasm_temp)
            .map_err(|e| format!("Failed to create WASM temp file {}: {}", wasm_temp.display(), e))?;
    ...
    fs::rename(&wasm_temp, &wasm_final)
        .map_err(|e| format!("Failed to atomically move WASM file {} to {}: {}", 
            wasm_temp.display(), wasm_final.display(), e))?;
    ...
    {
        let mut file = fs::File::create(&spec_temp)
            .map_err(|e| format!("Failed to create config temp file {}: {}", spec_temp.display(), e))?;
    ...
    fs::rename(&spec_temp, &spec_final)
        .map_err(|e| format!("Failed to atomically move config file {} to {}: {}", 
            spec_temp.display(), spec_final.display(), e))?;
...
/// Only serializes fields that are relevant for deployment config (version, namespace, supervisor).
                ...
                if !child.facets.is_empty() {
                    ...
                    for facet in &child.facets {
                        ...
                        if !facet.config.is_empty() {
                            let config_parts: Vec<String> = facet.config.iter()
...
mod tests {
    ...
    fn test_save_wasm_app_atomically_enabled() {
        ...
        assert!(!wasm_temp.exists(), "WASM temp file should not exist after atomic move");
        assert!(!config_temp.exists(), "Config temp file should not exist after atomic move");
