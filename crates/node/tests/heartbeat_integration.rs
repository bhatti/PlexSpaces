...
async fn test_node_registration_and_heartbeat() {
    ...
    let object_registry = node.service_locator().get_object_registry().await
    ...
    if let (Some(initial), Some(updated)) = (initial_heartbeat, &updated_registration.last_heartbeat) {
    ...
    // Shutdown
    node.shutdown(Duration::from_secs(5)).await.expect("Node should shutdown");
}
...
async fn test_heartbeat_across_nodes_same_cluster() {
    ...
    let object_registry1 = node1.service_locator().get_object_registry().await
        .expect("ObjectRegistry should be available");
    
    // Use internal context with cluster_name as namespace (same as registration)
    let ctx1 = node1.service_locator()
    ...
    // Shutdown
    node1.shutdown(Duration::from_secs(5)).await.expect("Node1 should shutdown");
    node2.shutdown(Duration::from_secs(5)).await.expect("Node2 should shutdown");
}
...
async fn test_cluster_isolation() {
    ...
    assert!(node2_lookup.is_none(), "Node2 should not be visible from Node1's cluster");
