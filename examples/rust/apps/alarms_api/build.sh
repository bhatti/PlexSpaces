...
# Build Alarms API WASM component using Rust
#
# Pipeline: cargo → core WASM → embed WIT → WASM Component
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
WIT_DIR="$REPO_ROOT/wit/plexspaces-actor"
OUTPUT_WASM="$SCRIPT_DIR/alarms_api_actor.wasm"
EMBEDDED_WASM="$SCRIPT_DIR/alarms_api_actor_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/alarms_api_wasm.wasm"
...
echo "Building alarms_api (Rust WASM)..."
