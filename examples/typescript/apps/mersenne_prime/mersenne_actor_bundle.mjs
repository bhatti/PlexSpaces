...
var CANDIDATES = [
  ...
  9689,
  9941
  // heavy  (seconds–tens of seconds)
];
var WORKER_GROUP = "mersenne_workers";
var WORKER_JS = `
...
`.trim();
var CoordinatorActor = class extends PlexSpacesActor {
  getDefaultState() {
  ...
  onReady(payload) {
    ...
    if (!next) {
      try {
        host.send(payload.actor_id, "assign_work", { p: null, done: true });
      } catch {
      }
      return { done: true };
    }
  ...
  onResult(payload) {
    ...
    if (actorId) {
      ...
      if (next) {
        ...
        if (this.state.workers[actorId])
          this.state.workers[actorId].current_p = void 0;
        try {
          host.send(actorId, "assign_work", { p: null, done: true });
        } catch {
        }
      }
    }
  ...
  onStatus() {
    const items = Object.values(this.state.work);
    ...
    const throughput_per_s = elapsed_ms > 0 ? done.length / elapsed_ms * 1e3 : 0;
    let pg_members = [];
    try {
      pg_members = host.processGroups.members(WORKER_GROUP) ?? [];
    } catch {
    }
    ...
    if (done.length === items.length && items.length > 0 && worker_count === 1 && elapsed_ms > 0 && this.state.single_worker_baseline_ms === 0) {
      this.state.single_worker_baseline_ms = elapsed_ms;
    }
    const speedup = this.state.single_worker_baseline_ms > 0 && elapsed_ms > 0 ? this.state.single_worker_baseline_ms / elapsed_ms : null;
    const efficiency_pct = speedup != null && worker_count > 0 ? Math.round(speedup / worker_count * 100) : null;
  ...
  _nextPending(cpuCores) {
    const pending = Object.values(this.state.work).filter((w) => w.status === "pending").sort((a, b) => cpuCores <= 2 ? a.p - b.p : b.p - a.p);
    return pending[0] ?? null;
  }
};
var CodeServerActor = class extends PlexSpacesActor {
  getDefaultState() {
    return {};
  }
  onGet_worker_js() {
