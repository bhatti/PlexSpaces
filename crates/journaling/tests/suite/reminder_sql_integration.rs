...
async fn test_query_due_reminders() {
    ...
    let registration1 =
        create_test_reminder_registration("actor1@node1", "due1", 60, now_secs - 10);
    let reminder_state1 = ReminderState {
        registration: Some(registration1.clone()),
        last_fired: None,
    ...
    let registration3 =
        create_test_reminder_registration("actor1@node1", "not_due", 60, now_secs + 100);
    ...
    let due = storage
        .query_due_reminders(now + Duration::from_secs(1))
        .await
        .unwrap();
    assert_eq!(due.len(), 2);

    let due_names: Vec<String> = due
        .iter()
        .map(|r| r.registration.as_ref().unwrap().reminder_name.clone())
        .collect();
...
async fn test_query_due_reminders_only_active() {
    ...
    let registration1 =
        create_test_reminder_registration("actor1@node1", "active_due", 60, now_secs - 10);
    let reminder_state1 = ReminderState {
        registration: Some(registration1.clone()),
        last_fired: None,
        next_fire_time: Some(prost_types::Timestamp {
    ...
    let registration2 =
        create_test_reminder_registration("actor1@node1", "inactive_due", 60, now_secs - 10);
    let reminder_state2 = ReminderState {
        registration: Some(registration2.clone()),
        last_fired: None,
        next_fire_time: Some(prost_types::Timestamp {
    ...
    assert_eq!(due.len(), 1);
    assert_eq!(
        due[0].registration.as_ref().unwrap().reminder_name,
        "active_due"
    );
}
...
async fn test_query_due_reminders_ordering() {
    ...
    assert_eq!(
        due[1].registration.as_ref().unwrap().reminder_name,
        "reminder2"
    );
    assert_eq!(
        due[2].registration.as_ref().unwrap().reminder_name,
        "reminder1"
    );
}
...
async fn test_reminder_with_nanos_precision() {
    ...
    assert_eq!(
        loaded[0].next_fire_time.as_ref().unwrap().nanos,
        250_000_000
    );
}
