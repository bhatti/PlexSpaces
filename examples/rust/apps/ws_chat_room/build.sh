...
OUTPUT_WASM="$SCRIPT_DIR/ws_chat_room_actor.wasm"
EMBEDDED_WASM="$SCRIPT_DIR/ws_chat_room_actor_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/rust_ws_chat_room.wasm"
...
echo "Building ws_chat_room (Rust WASM)..."
...
for candidate in \
  ...
  "$REPO_ROOT/examples/typescript/apps/ws_chat_room/node_modules/@bytecodealliance/jco/lib/wasi_snapshot_preview1.reactor.wasm" \
  "$REPO_ROOT/examples/typescript/apps/chat_room/node_modules/@bytecodealliance/jco/lib/wasi_snapshot_preview1.reactor.wasm" \
