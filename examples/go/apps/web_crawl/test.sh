...
TARGET_DIR="$REPO_ROOT/target/examples/go/web_crawl"
...
print(f"  Throughput        : {tps:.1f} pages/sec")
print(f"  Parallel fraction : {parallel:.2%}  (1 - coord/elapsed)")
print(f"  ────────────────────────────────────────────────────────")
print("  Top words         :")
for entry in words:
...
print("          Parallel frac = 1 - coord/elapsed (Amdahl's Law serial fraction = 1 - parallel_frac)")
print("          Word data = total word occurrences / unique vocabulary per round")
print()

# Worker fetch distribution for last round
if results:
    last = results[-1]
    fetches = last.get("worker_fetches", [])
    if any(f > 0 for f in fetches):
        print(f"  Worker fetch distribution ({last['workers']} workers, {last['pages']} pages):")
        for i, fc in enumerate(fetches):
            bar = "█" * min(fc // 2, 40)
            print(f"    fetcher-{i:>2}: {fc:>4}  {bar}")
PY

echo ""
echo "================================================================"
echo -e "  ${GREEN}Web Crawl (Go WASM) Test Complete${NC}"
echo ""
...
echo "    ✓ ShardGroup scatter   — interleaved scatter to 4 analyzer shards"
echo "    ✓ Scaling benchmark    — 1/4/8/16 workers, pages/sec, speedup, Amdahl efficiency"
echo "    ✓ Real work            — 12 links/page, 80+ words/page, 200-page corpus per round"
echo "================================================================"
