...
echo "Step 2: Reset (default two-body)"
R0=$(send_op '{"op":"reset"}' 15)
assert_actor_ok "reset" "$R0"

echo "Step 3: Integrate a few steps"
for _ in 1 2 3 4 5; do
  RS=$(send_op '{"op":"step","dt":0.01}' 15)
  assert_actor_ok "step" "$RS"
done

echo "Step 4: get_state"
ST=$(send_op '{"op":"get_state"}' 15)
assert_actor_ok "get_state" "$ST"
echo "  bodies: $(echo "$ST" | head -c 120)..."

echo "Step 5: Burst run_steps"
BATCH_START=$(date +%s%N)
RUN_BURST=$(send_op '{"op":"run_steps","count":30,"dt":0.005}' 60)
BATCH_END=$(date +%s%N)
WALL_MS=$(( (BATCH_END - BATCH_START) / 1000000 ))
assert_actor_ok "run_steps" "$RUN_BURST"
echo -e "  ${GREEN}Burst wall: ${WALL_MS}ms${NC}"

echo "Step 6: Metrics (get_status + application list)"
...
print(f"  Burst wall (step 5): {wall_ms}ms")
