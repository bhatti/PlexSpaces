...
use tonic_health::server::{health_reporter, HealthReporter as TonicHealthReporter};

// Make PlexSpacesHealthReporter implement Service trait for ServiceLocator registration
impl crate::Service for PlexSpacesHealthReporter {
    fn service_name(&self) -> String {
        "PlexSpacesHealthReporter".to_string()
    }
}

// Implement HealthReporter trait for PlexSpacesHealthReporter
#[async_trait::async_trait]
impl HealthReporter for PlexSpacesHealthReporter {
    async fn is_alive(&self) -> bool {
        PlexSpacesHealthReporter::is_alive(self).await
    }
    ...
    async fn get_readiness(&self) -> NodeReadinessStatus {
        PlexSpacesHealthReporter::get_readiness(self).await
    }

    async fn get_state(&self) -> NodeHealthState {
        PlexSpacesHealthReporter::get_state(self).await
    }

    async fn get_detailed_health(&self, include_non_critical: bool) -> DetailedHealthCheck {
        PlexSpacesHealthReporter::get_detailed_health(self, include_non_critical).await
    }

    async fn mark_startup_complete(&self, message: Option<String>) -> prost_types::Duration {
        let duration = PlexSpacesHealthReporter::mark_startup_complete(self, message).await;
    ...
    async fn set_service_status(&self, service_name: &str, status: ServingStatus) {
        PlexSpacesHealthReporter::set_service_status(self, service_name, status).await
    }

    async fn get_service_status(&self, service_name: &str) -> ServingStatus {
        PlexSpacesHealthReporter::get_service_status(self, service_name).await
    }

    async fn get_all_service_statuses(&self) -> std::collections::HashMap<String, ServingStatus> {
        PlexSpacesHealthReporter::get_all_service_statuses(self).await
    }

    async fn is_shutting_down(&self) -> bool {
        PlexSpacesHealthReporter::is_shutting_down(self).await
    }

    async fn update_in_flight_requests(&self, count: u64) {
        PlexSpacesHealthReporter::update_in_flight_requests(self, count).await
    }

    async fn is_ready(&self) -> bool {
        PlexSpacesHealthReporter::is_ready(self).await
    }
}
