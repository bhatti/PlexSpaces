...
//! Parse JSON byte spawn-init payloads into role hints and flat argument maps.
//!
//! [`crate::virtual_actor_manager::wasm_init_payload`] builds the canonical JSON shape for
//! WASM and BehaviorRegistry (`actor_id`, `actor_type`, `role`, `behavior_kind`,
...
//! **`role`** (from `role` or `config.role`) and **`args`** so callers can
...
mod tests {
    ...
    #[test]
    fn nested_config_role_fills_role_hint() {
        let json = br#"{"config":{"role":"leader"},"args":{"k":"v"}}"#;
        let (role, args) = legacy_spawn_init_json_to_role_and_args(json);
    ...
    #[test]
    fn top_level_role_takes_priority_over_config_role() {
        let json = br#"{"role":"worker","config":{"role":"leader"}}"#;
        let (role, _) = legacy_spawn_init_json_to_role_and_args(json);
        assert_eq!(role.as_deref(), Some("worker"));
    }

    #[test]
    fn meta_keys_excluded_from_promoted_args() {
        // role, actor_id, actor_type, behavior_kind, args, config must never leak into user args.
        let json = br#"{"role":"worker","actor_id":"x","actor_type":"y","behavior_kind":"GenServer","custom":"val"}"#;
        let (_, args) = legacy_spawn_init_json_to_role_and_args(json);
        assert!(!args.contains_key("role"));
        assert!(!args.contains_key("actor_id"));
        assert!(!args.contains_key("actor_type"));
        assert!(!args.contains_key("behavior_kind"));
        assert_eq!(args.get("custom").map(String::as_str), Some("val"));
    }

    #[test]
    fn empty_role_string_not_returned() {
        let json = br#"{"role":""}"#;
        let (role, _) = legacy_spawn_init_json_to_role_and_args(json);
        assert!(role.is_none(), "empty role string should yield None");
    }
}
