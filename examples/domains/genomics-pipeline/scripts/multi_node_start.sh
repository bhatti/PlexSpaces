...
echo -e "${BLUE}=== Starting Genomics Pipeline Multi-Node Cluster ===${NC}"
echo ""

cd "$PROJECT_DIR"

# Create PID directory
mkdir -p /tmp/genomics-pipeline
...
# Build binaries
echo -e "${BLUE}Building genomics-node binary...${NC}"
cargo build --release --bin genomics-node

GENOMICS_NODE="../../target/release/genomics-node"

if [ ! -f "$GENOMICS_NODE" ]; then
    echo -e "${RED}Error: genomics-node binary not found${NC}"
    echo "Expected: $GENOMICS_NODE"
    exit 1
fi
...
# Start Node 4 first (annotation workers - leaf dependency)
echo -e "${BLUE}Starting Node 4 (Annotation + Reporting)...${NC}"
RUST_LOG=info $GENOMICS_NODE --config config/node4.toml \
    > logs/node4.log 2>&1 &
echo $! > "$PID_DIR/node4.pid"
echo -e "${GREEN}  Node 4 started (PID: $(cat $PID_DIR/node4.pid))${NC}"
sleep 2

# Start Node 3 (chromosome workers)
echo -e "${BLUE}Starting Node 3 (Chromosome Workers)...${NC}"
RUST_LOG=info $GENOMICS_NODE --config config/node3.toml \
    > logs/node3.log 2>&1 &
echo $! > "$PID_DIR/node3.pid"
echo -e "${GREEN}  Node 3 started (PID: $(cat $PID_DIR/node3.pid))${NC}"
sleep 2

# Start Node 2 (QC + alignment workers)
echo -e "${BLUE}Starting Node 2 (QC + Alignment Workers)...${NC}"
RUST_LOG=info $GENOMICS_NODE --config config/node2.toml \
    > logs/node2.log 2>&1 &
...
RUST_LOG=info $GENOMICS_NODE --config config/node1.toml \
    > logs/node1.log 2>&1 &
...
echo "  Check status:    ps aux | grep genomics-node"
