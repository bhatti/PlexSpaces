...
impl BankAccount {
    ...
    fn add_transaction(&mut self, transaction_type: TransactionType, amount: f64, description: String) {
        let now = chrono::Utc::now().timestamp();
...
impl BankAccount {
    ...
    #[handler("deposit")]
    async fn handle_deposit(&mut self, _ctx: &ActorContext, msg: &Message) -> Result<Value, BehaviorError> {
        #[derive(Deserialize)]
        ...
        let req: DepositRequest = serde_json::from_slice(&msg.payload)
            .map_err(|e| BehaviorError::ProcessingError(format!("Invalid deposit request: {}", e)))?;
        
        if req.amount <= 0.0 {
            return Err(BehaviorError::ProcessingError("Deposit amount must be positive".to_string()));
        }
    ...
    /// - State changes persisted to journal storage
    #[handler("withdraw")]
    async fn handle_withdraw(&mut self, _ctx: &ActorContext, msg: &Message) -> Result<Value, BehaviorError> {
        #[derive(Deserialize)]
        ...
        let req: WithdrawRequest = serde_json::from_slice(&msg.payload)
            .map_err(|e| BehaviorError::ProcessingError(format!("Invalid withdraw request: {}", e)))?;
        
        if req.amount <= 0.0 {
            return Err(BehaviorError::ProcessingError("Withdrawal amount must be positive".to_string()));
        }
    ...
    /// ```
    #[handler("get_balance")]
    async fn handle_get_balance(&mut self, _ctx: &ActorContext, _msg: &Message) -> Result<Value, BehaviorError> {
        Ok(json!({
    ...
    async fn handle_get_transactions(&mut self, _ctx: &ActorContext, msg: &Message) -> Result<Value, BehaviorError> {
        #[derive(Deserialize)]
        struct TransactionQuery {
            limit: Option<usize>,
        }
        
        let query: TransactionQuery = serde_json::from_slice(&msg.payload)
            .unwrap_or(TransactionQuery { limit: None });
        
        let limit = query.limit.unwrap_or(self.state.transactions.len());
        let transactions: Vec<&Transaction> = self.state.transactions
            .iter()
            .rev()
            .take(limit)
            .collect();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  ✓ Journal storage created ({:.2}ms)", storage_time.as_secs_f64() * 1000.0);
    ...
    // Architecture: spawn_with_storage() delegates to ActorFactoryImpl internally
    // Core functionality (DurabilityFacet creation) lives in main crates
    // SDK provides convenience wrapper to simplify usage
    ...
    println!("  ✓ Account '{}' spawned ({:.2}ms)", account_id, spawn_time.as_secs_f64() * 1000.0);
    ...
    // =========================================================================
    println!("Step 4: Process {} Transactions (All Journaled)", num_transactions);
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    ...
    let deposit_result: Value = account.call("deposit", &json!({
        "amount": initial_deposit,
        "description": "Initial deposit"
    })).await?;
    
    println!("  Initial deposit: ${:.2}", initial_deposit);
    println!("  Balance after deposit: ${:.2}", deposit_result["balance"].as_f64().unwrap());
    println!();
    ...
    for i in 0..num_transactions {
        ...
        if is_deposit {
            let _: Value = account.call("deposit", &json!({
                "amount": amount,
            ...
            if balance >= amount {
                let _: Value = account.call("withdraw", &json!({
                    "amount": amount,
        ...
        if i < 5 || i >= num_transactions - 5 {
            let balance_result: Value = account.call("get_balance", &json!({})).await?;
            println!("  Transaction {}: {} ${:.2} → Balance: ${:.2}",
                i + 1,
    ...
    println!("  Processed {} transactions in {:.2}ms", num_transactions, process_time.as_secs_f64() * 1000.0);
    ...
    let history_result: Value = account.call("get_transactions", &json!({
        "limit": 10
    })).await?;
    ...
    println!("Transactions/Second:            {:.2}", transactions_per_sec);
    ...
    if metrics.efficiency >= 0.9 {
        ...
        println!("⚠ Efficiency below 90% - consider batching transactions or optimizing journal writes");
    }
    println!();
    ...
    println!("  • spawn_with_storage() - SDK helper (delegates to ActorFactoryImpl with DurabilityFacet)");
