#!/bin/bash
# Test HPC Ensemble - EFlows4HPC style (use Python/TypeScript/Go-built WASM)
# Single actor type: ensemble:coord-1 (coordinator), ensemble:worker-0/1 (workers)
# Usage: ./test.sh [HTTP_PORT]
# If ensemble_actor.wasm is not in this dir, try ../../../python/apps/migrating_eflows4hpc/ensemble_actor.wasm
set -euo pipefail
...
# Prefer local WASM; else Python example's WASM
WASM_FILE="$SCRIPT_DIR/ensemble_actor.wasm"
if [ ! -f "$WASM_FILE" ]; then
  WASM_FILE="$SCRIPT_DIR/../../../python/apps/migrating_eflows4hpc/ensemble_actor.wasm"
fi
if [ ! -f "$WASM_FILE" ]; then
  echo "ERROR: No ensemble_actor.wasm found. Build Python example: cd examples/python/apps/migrating_eflows4hpc && ./build.sh"
  exit 1
fi
...
echo "================================================================"
echo "  EFlows4HPC → PlexSpaces: HPC Ensemble (Rust example – shared config)"
echo "  WASM: $WASM_FILE"
echo "================================================================"
