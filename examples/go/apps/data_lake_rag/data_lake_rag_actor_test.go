...
func TestActorIDHelpers(t *testing.T) {
	actorID := "01KM1FJ3AF3BA2BDNKZPGH9K2P//worker::data-lake-rag-go@test-node-8093"
	base := &plexspaces.BaseActor{}
	base.SetRuntimeMetadata(actorID)
	if got := base.ApplicationID(); got != "data-lake-rag-go" {
		t.Fatalf("ApplicationID() = %q, want %q", got, "data-lake-rag-go")
	}
...
func TestComputeActorCountsAssignsLeaderAndWorkersToNodes(t *testing.T) {
	counts := computeActorCounts(
		"test-node-8091",
		[]string{
			"01A//worker::data-lake-rag-go@test-node-8091",
			"01B//worker::data-lake-rag-go@test-node-8093",
		},
	)
