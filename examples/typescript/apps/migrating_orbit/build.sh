#!/usr/bin/env bash
# Build Read State Tracker WASM actor: TypeScript → JS → ESM bundle → WASM component
set -euo pipefail
...
echo "Building TypeScript read_state → WASM..."
echo ""
...
if [ ! -f "read_state_actor.js" ]; then
  echo "  ERROR: read_state_actor.js not produced"
  exit 1
fi
echo "  ✓ read_state_actor.js"
