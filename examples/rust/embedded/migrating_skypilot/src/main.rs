    ...
    fn find_best_resources(&self, task: &AITask) -> Option<ResourceAllocation> {
        ...
        for (instance_id, instance) in &self.cloud_catalog {
            ...
            let meets_cloud = task.cloud_preference.is_none()
                || instance_id
                    .starts_with(&format!("{}:", task.cloud_preference.as_ref().unwrap()));
    ...
    async fn handle_request(
        ...
        let reply = match sky_msg {
            SkyPilotMessage::SubmitTask { task } => {
                info!(
                    "[SKYPILOT] Submitting AI task: {} (type: {}, gpu: {})",
                ...
                if let Some(allocation) = self.find_best_resources(&task) {
                    ...
                    info!(
                        "[SKYPILOT] Scheduled task on {} ({}) - cost: ${}/hr",
                        allocation.cloud_provider,
                    ...
                    return Err(BehaviorError::ProcessingError(
                        "No resources available, queued".to_string(),
                    ));
                }
            }
            SkyPilotMessage::GetBestResources { task } => {
                if let Some(allocation) = self.find_best_resources(&task) {
                    ...
                    return Err(BehaviorError::ProcessingError(
                        "No suitable resources found".to_string(),
                    ));
                }
            }
            SkyPilotMessage::GetStatus => SkyPilotMessage::Status {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let SkyPilotMessage::TaskScheduled { allocation } = reply {
        info!(
            "✅ Inference task scheduled: {} on {} ({}) - ${}/hr",
            allocation.task_id,
            allocation.cloud_provider,
            allocation.instance_type,
            allocation.cost_per_hour
