...
use plexspaces_core::{ServiceLocator, RequestContext, ActorRegistry};
...
async fn test_is_actor_local_registered_locally() {
    ...
    registry.register_actor(&ctx, actor_id.clone(), Arc::new(actor_ref), None, None, None, None).await;
    ...
    assert!(is_local, "Actor registered locally should be detected as local");
}
...
async fn test_is_actor_local_no_node_config() {
    ...
    assert!(is_local, "Should use ActorRegistry fallback when NodeConfig not available");
}
