...
OUTPUT_WASM="$SCRIPT_DIR/minihermes_actor.wasm"

cd "$SCRIPT_DIR"

echo "Building MiniHermes (Go WASM)..."
...
go mod tidy
tinygo build -target=wasi -o minihermes_actor_core.wasm .
wasm-tools component embed "$WIT_DIR" -w actor-world minihermes_actor_core.wasm -o minihermes_actor_embedded.wasm
wasm-tools component new minihermes_actor_embedded.wasm --adapt "wasi_snapshot_preview1=$ADAPTER" -o "$OUTPUT_WASM"
rm -f minihermes_actor_core.wasm minihermes_actor_embedded.wasm
