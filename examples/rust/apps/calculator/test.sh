...
echo "Step 2: Basic ops"
R1=$(send_op '{"op":"calculate","operation":"add","operands":[10,3]}' 15)
assert_actor_ok "add" "$R1"
echo "$R1" | grep -q '"result"' || { echo "$R1"; exit 1; }

R2=$(send_op '{"op":"calculate","operation":"subtract","operands":[10,3]}' 15)
assert_actor_ok "subtract" "$R2"

R3=$(send_op '{"op":"calculate","operation":"multiply","operands":[6,7]}' 15)
assert_actor_ok "multiply" "$R3"

R4=$(send_op '{"op":"calculate","operation":"divide","operands":[100,4]}' 15)
assert_actor_ok "divide" "$R4"
echo -e "  ${GREEN}Basic ops OK${NC}"

echo "Step 3: Division by zero (expect error in payload)"
BAD=$(send_op '{"op":"calculate","operation":"divide","operands":[1,0]}' 15)
if ! echo "$BAD" | grep -q 'division by zero'; then
  echo -e "${RED}Expected division error in response${NC}"
  echo "$BAD"
  exit 1
fi
echo -e "  ${GREEN}Error path OK${NC}"

echo "Step 4: get_stats"
ST=$(send_op '{"op":"get_stats"}' 15)
assert_actor_ok "get_stats" "$ST"
echo "  stats: $(echo "$ST" | head -c 160)..."

echo "Step 5: batch"
BATCH_START=$(date +%s%N)
OPS_JSON='{"op":"batch","operations":['
for i in $(seq 1 25); do
  [ "$i" -gt 1 ] && OPS_JSON+=','
  OPS_JSON+="{\"operation\":\"add\",\"operands\":[$i,1]}"
done
OPS_JSON+=']}'
BURST=$(send_op "$OPS_JSON" 60)
BATCH_END=$(date +%s%N)
...
print(f"  Batch wall (step 5): {wall_ms}ms")
...
print_metric_maps(m, "calculator")
PY
then
  echo -e "${RED}Step 6 metrics report failed.${NC}"
  exit 1
fi
