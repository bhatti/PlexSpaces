...
use crate::{Facet, FacetContainer, FacetError, FacetRegistry};
...
pub async fn create_facet_from_json(
    ...
    let facet = facet_registry.create_facet(facet_type, config).await?;
...
pub async fn create_facets_from_config(
    ...
    if let Some(config_obj) = facet_config.as_object() {
        ...
        let has_facet_type_keys = config_obj
            .keys()
            .any(|k| known_facet_types.contains(&k.as_str()));
        ...
        if has_facet_type_keys {
            ...
            if let Ok(facet) =
                create_facet_from_json("virtual_actor", facet_config.clone(), facet_registry).await
            {
                facets.push(facet);
            }
        }
...
mod tests {
    ...
    fn test_extract_facet_config() {
        ...
        assert_eq!(
            config_obj.get("idle_timeout").unwrap().as_str().unwrap(),
            "5m"
        );
    }
