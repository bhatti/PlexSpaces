#!/usr/bin/env bash
# Build Webhook Processor WASM component (Go/TinyGo) - AWS Durable Lambda style
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
WIT_DIR="$REPO_ROOT/wit/plexspaces-actor"
OUTPUT_WASM="$SCRIPT_DIR/webhook_actor.wasm"

cd "$SCRIPT_DIR"

echo "Building webhook processor (Go - AWS Durable Lambda style)..."
...
rm -f webhook_core.wasm webhook_embedded.wasm
