...
class ChatAgentActor extends PlexSpacesActor<ChatAgentState> {
  // How many messages before we set the summarization alarm.
  private static readonly ALARM_THRESHOLD = 10;
  // Summarization alarm delay: 5 minutes in milliseconds.
  private static readonly ALARM_DELAY_MS = 300_000;

  getDefaultState(): ChatAgentState {
    return {
      agent_id: "",
      total_messages: 0,
  ...
  onChat(payload: Record<string, unknown>): Record<string, unknown> {
  ...
  // ---------- get_history handler ----------
  onGet_history(_payload: Record<string, unknown>): Record<string, unknown> {
    let history: ChatMessage[] = [];
    try {
      const raw = host.kv.get("history");
      if (raw) {
  ...
  onClear(_payload: Record<string, unknown>): Record<string, unknown> {
