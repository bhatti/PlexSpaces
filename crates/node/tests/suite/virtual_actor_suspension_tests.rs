...
impl StateLoader for CounterStateLoader {
    fn deserialize(&self, state_data: &[u8]) -> JournalResult<JsonValue> {
        ...
        let count =
            u32::from_le_bytes(state_data[0..4].try_into().map_err(|_| {
...
async fn test_suspend_active_virtual_actor_then_tell() {
    ...
    assert!(
        is_active,
        "Eager actor should be active immediately (synchronous activation)"
    );
    ...
    assert!(
        is_virtual_final,
        "Actor should still be virtual after tell()"
    );
    assert!(
        is_active_final,
        "Actor should be active again after tell() (synchronous activation)"
    );
