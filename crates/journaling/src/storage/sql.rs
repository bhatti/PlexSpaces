...
impl PostgresJournalStorage {
    ...
    pub async fn new(connection_string: &str) -> JournalResult<Self> {
        ...
        // Create journal_entries table with JSONB for extensibility
        sqlx::query(
            r#"
            CREATE TABLE IF NOT EXISTS journal_entries (
                id TEXT PRIMARY KEY,
        ...
        // Add state_schema_version column if it doesn't exist (for existing databases)
        // PostgreSQL: Use DO block to check and add column
        #[cfg(feature = "postgres-backend")]
        {
            let _ = sqlx::query(
                r#"
                ...
                BEGIN
                    IF NOT EXISTS (
                        ...
                        WHERE table_name = 'checkpoints' AND column_name = 'state_schema_version'
                    ) THEN
                        ALTER TABLE checkpoints ADD COLUMN state_schema_version INTEGER NOT NULL DEFAULT 1;
                    END IF;
                END $$;
                "#,
            )
