...
OUTPUT_WASM="$SCRIPT_DIR/parallel_ai_inference_actor.wasm"
EMBEDDED_WASM="$SCRIPT_DIR/parallel_ai_inference_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/parallel_ai_inference_wasm.wasm"
...
echo "Building Parallel AI Inference (Rust WASM)..."
