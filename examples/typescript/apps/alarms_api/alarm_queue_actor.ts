...
class RequestQueueActor extends PlexSpacesActor<RequestQueueState> {
  getDefaultState(): RequestQueueState {
    return {
      items: [],
      count: 0,
      total_processed: 0,
  ...
  onEnqueue(payload: Record<string, unknown>): Record<string, unknown> {
    ...
    if (wasEmpty) {
      ...
      const fireAt = host.nowMs() + 10_000;
