...
mod sqlite_tests {
    ...
    async fn test_checkpoint_scenario() {
        ...
        assert_eq!(
            all_entries.len(),
            200,
            "Should have exactly 200 entries (100 messages * 2)"
        );
        ...
        assert_eq!(
            checkpoint_after_restart.sequence, checkpoint.sequence,
    ...
    async fn test_replay_doesnt_lose_entries() {
        ...
        assert_eq!(
            entries2.len(),
            6,
            "Should still have 6 entries after detach"
        );
        ...
        assert_eq!(
            entries3.len(),
            6,
            "Should still have 6 entries after reattach with replay"
        );
    }
}
