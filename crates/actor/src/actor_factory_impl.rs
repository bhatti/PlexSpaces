...
impl ActorFactory for ActorFactoryImpl {
        ...
        if !evicted.is_empty() {
            ...
            for evicted_actor_id in &evicted {
                ...
                let (tenant_id, namespace) =
                    if let Some(metadata) = manager.get_metadata(evicted_actor_id).await {
        ...
        if let Some(actor_any) = registry.get_actor_instance(&actor_id).await {
            ...
            if let Some(facet_arc) = metadata.facet.clone() {
                ...
                if let Some(strategy) = activation_strategy.clone() {
                    ...
                    let virtual_actor_config = facet_config
                        .get("virtual_actor")
                        .cloned()
                        .unwrap_or_else(|| serde_json::json!({}));
                    let virtual_facet_new = Box::new(VirtualActorFacet::new(
                        virtual_actor_config,
                    ...
                    if let Some(facet_metadata) = metadata.get(facet_type) {
                        ...
                        if facet_type == "durability" {
                            ...
                            if let Some(storage) = self.service_locator.get_journal_storage().await
                            {
                                ...
                                let new_facet =
                                    Box::new(DurabilityFacet::new(storage, config, priority));
                                facets_to_attach
                                    .push(new_facet as Box<dyn plexspaces_facet::Facet>);
                            } else {
                                tracing::warn!(
                                    actor_id = %actor_id,
            ...
            if is_lazy {
                ...
                if let Some(actor_any) = registry.get_actor_instance(&actor_id).await {
                    ...
                    registry
                        ...
                        .await
                        .map_err(|e| {
                            format!("Failed to unregister actor before activation: {}", e)
                        })?;

                    let mut actor = Arc::try_unwrap(actor_arc).map_err(|_| {
                        "Actor Arc has multiple references - cannot unwrap for activation"
                    })?;

                    use crate::ActorState;
                    let current_state = actor.state().await;
                    if current_state == ActorState::Creating
                        || current_state == ActorState::Terminated
                    {
                        let join_handle = actor.start().await.map_err(|e| {
                            format!("Failed to start reactivated lazy virtual actor: {}", e)
                        })?;
                        ...
                        let facet_manager_wrapper = self
                            ...
                            .await
                            .ok_or_else(|| {
                                "FacetManager not found in ServiceLocator".to_string()
                            })?;
                        let facet_manager = facet_manager_wrapper.inner_clone();
                        let facets_clone = actor_arc.facets();
                        facet_manager
                ...
                manager.mark_activated(&actor_id).await.map_err(|e| {
                    format!("Failed to mark rebuilt eager actor as activated: {}", e)
                })?;
    ...
    async fn spawn_actor(
        ...
        let actor_ref = self
            .spawn_built_actor_impl(ctx, Arc::new(actor), Some(actor_type.to_string()))
            .await?;
    ...
    pub async fn spawn_built_actor_impl(
        ...
        if is_virtual {
            ...
            if instance_metadata.is_none() {
                ...
                let actor_type_str = actor_type.clone().ok_or_else(|| {
                    format!("Actor type is required for virtual actor registration")
                })?;
...
impl ActorFactoryImpl {
        ...
        B: plexspaces_core::Actor + Send + 'static,
    {
        ...
        self.spawn_built_actor_impl(ctx, Arc::new(actor), Some(actor_type.to_string()))
            .await
    }
}
