...
mod tests {
    ...
    async fn test_discover_nodes_caching() {
        ...
        // Register a node
        register_node(&registry, &ctx, "node1", "http://127.0.0.1:8000", None)
            .await
            .unwrap();
    ...
    async fn test_discover_nodes_cache_invalidation() {
        ...
        register_node(&registry, &ctx, "node2", "http://127.0.0.1:8001", None)
            .await
            .unwrap();
    ...
    async fn test_discover_application_nodes_basic() {
        ...
        let nodes = discover_application_nodes(&registry, &ctx, "myapp")
            .await
            .unwrap();
    ...
    async fn test_discover_application_nodes_caching() {
        ...
        // First discovery
        let nodes1 = discover_application_nodes(&registry, &ctx, "myapp")
            .await
            .unwrap();
        assert_eq!(nodes1.len(), 1);

        // Second discovery - should use cache
        let nodes2 = discover_application_nodes(&registry, &ctx, "myapp")
            .await
            .unwrap();
        assert_eq!(nodes2.len(), 1);
        assert_eq!(nodes1[0].object_id, nodes2[0].object_id);
    ...
    async fn test_discover_workflow_nodes_basic() {
        ...
        let nodes = discover_workflow_nodes(&registry, &ctx, "def1")
            .await
            .unwrap();
        assert_eq!(nodes.len(), 2);
    ...
    async fn test_discover_workflow_nodes_caching() {
        ...
        // First discovery
        let nodes1 = discover_workflow_nodes(&registry, &ctx, "def1")
            .await
            .unwrap();
        assert_eq!(nodes1.len(), 1);

        // Second discovery - should use cache
        let nodes2 = discover_workflow_nodes(&registry, &ctx, "def1")
            .await
            .unwrap();
        assert_eq!(nodes2.len(), 1);
        assert_eq!(nodes1[0].object_id, nodes2[0].object_id);
    ...
    async fn test_multi_tenant_isolation() {
        ...
        register_node(&registry, &ctx1, "node1", "http://127.0.0.1:8000", None)
            .await
            .unwrap();

        let ctx2 =
            RequestContext::new_without_auth("tenant2".to_string(), "namespace2".to_string());
        register_node(&registry, &ctx2, "node2", "http://127.0.0.1:8001", None)
            .await
            .unwrap();
    ...
    async fn test_discover_empty_results() {
        ...
        let apps = discover_application_nodes(&registry, &ctx, "nonexistent")
            .await
            .unwrap();
        assert_eq!(apps.len(), 0);

        // Discover workflow that doesn't exist
        let workflows = discover_workflow_nodes(&registry, &ctx, "nonexistent")
            .await
            .unwrap();
    ...
    async fn test_register_node_with_cluster() {
        ...
        let registration = registry
            .lookup_full(&ctx, ObjectType::ObjectTypeNode, "node1")
            .await
            .unwrap();
    ...
    async fn test_register_application() {
        ...
        // Verify registration
        let registration = registry
            .lookup_full(&ctx, ObjectType::ObjectTypeApplication, "myapp@node1")
            .await
            .unwrap();
        assert!(registration.is_some());
        let reg = registration.unwrap();
    ...
    async fn test_register_workflow() {
        ...
        // Verify registration
        let registration = registry
            .lookup_full(&ctx, ObjectType::ObjectTypeWorkflow, "workflow1")
            .await
            .unwrap();
        assert!(registration.is_some());
        let reg = registration.unwrap();
    ...
    async fn test_discover_multiple_applications() {
        ...
        let app1_nodes = discover_application_nodes(&registry, &ctx, "app1")
            .await
            .unwrap();
        assert_eq!(app1_nodes.len(), 2);

        // Discover app2 nodes - should find only node1
        let app2_nodes = discover_application_nodes(&registry, &ctx, "app2")
            .await
            .unwrap();
    ...
    async fn test_discover_multiple_workflows() {
        ...
        let def1_nodes = discover_workflow_nodes(&registry, &ctx, "def1")
            .await
            .unwrap();
        assert_eq!(def1_nodes.len(), 2);

        // Discover def2 workflows - should find only node1
        let def2_nodes = discover_workflow_nodes(&registry, &ctx, "def2")
            .await
            .unwrap();
