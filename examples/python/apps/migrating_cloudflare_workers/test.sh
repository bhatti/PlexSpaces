...
# Test Guild Chat - Discord-style Real-Time Chat Server (Python WASM)
#
# Usage: ./test.sh [HTTP_PORT]
#   HTTP_PORT: PlexSpaces HTTP gateway port (default: 8094)
#
# Prerequisites: Start PlexSpaces node first:
#   PLEXSPACES_JWT_SECRET=test cargo run -p plexspaces-cli -- start --node-id test-node --listen-addr 0.0.0.0:8094
...
APP_ID="py-cloudflare-guild-chat"
NUM_USERS=5
BATCH_MSGS=200
...
echo ""
echo "Configuration:"
echo "  HTTP Gateway:      $HTTP_HOST:$HTTP_PORT"
...
if [ "$HTTP_CHECK" = "000" ]; then
    ...
    echo "Start node first: PLEXSPACES_JWT_SECRET=test cargo run -p plexspaces-cli -- start --node-id test-node --listen-addr 0.0.0.0:$HTTP_PORT"
    exit 1
fi
...
# Undeploy any existing deployment
...
for _attempt in 1 2 3; do
  DEPLOY_OUT=$(curl -s -w "\n%{http_code}" -X POST "http://$HTTP_HOST:$HTTP_PORT/api/v1/applications/deploy" \
      ...
      -F "name=py-cloudflare-guild-chat" \
      -F "version=1.0.0" \
...
if echo "$RESPONSE" | grep -qi '"success".*true'; then
    ...
    echo "  - ChatRoomActor:    chat-room"
    echo "  - RateLimiterActor: rate-limiter"
    echo "  - AlarmDemoActor:   alarm-demo"
else
    echo -e "${RED}Deploy failed: $RESPONSE${NC}"
    exit 1
fi
...
send_op() {
    ...
    local payload="$2"
    local timeout="${3:-60}"
    curl -s --max-time "$timeout" -X POST "http://$HTTP_HOST:$HTTP_PORT/api/v1/actors/$APP_ID/$actor/ask?timeout=$timeout" \
...
CHAT_RESP=$(send_op "ChatRoomActor" '{"op":"stats"}')
if echo "$CHAT_RESP" | grep -q '"status":"ok"'; then
    echo -e "  ${GREEN}ChatRoomActor OK${NC}"
else
    echo -e "  ${RED}ChatRoomActor failed: $CHAT_RESP${NC}"
    exit 1
fi

RL_RESP=$(send_op "RateLimiterActor" '{"op":"status"}')
if echo "$RL_RESP" | grep -q '"status":"ok"'; then
    echo -e "  ${GREEN}RateLimiterActor OK${NC}"
else
    echo -e "  ${RED}RateLimiterActor failed: $RL_RESP${NC}"
    exit 1
fi
echo ""
...
for i in $(seq 1 $NUM_USERS); do
    RESP=$(send_op "ChatRoomActor" "{\"op\":\"join\",\"user_id\":\"user-$i\"}" 5)
...
for i in $(seq 1 $NUM_USERS); do
    RESP=$(send_op "ChatRoomActor" "{\"op\":\"send_message\",\"user_id\":\"user-$i\",\"content\":\"Hello from user-$i!\"}" 5)
...
MEMBERS_RESP=$(send_op "ChatRoomActor" '{"op":"get_members"}' 5)
...
HIST_RESP=$(send_op "ChatRoomActor" '{"op":"get_history","limit":10}' 5)
...
LEAVE_RESP=$(send_op "ChatRoomActor" '{"op":"leave","user_id":"user-1"}' 5)
...
DENIED=0
for i in $(seq 1 7); do
    RESP=$(send_op "RateLimiterActor" '{"op":"check","user_id":"test-user"}' 5)
...
ALARM_RESP=$(send_op "AlarmDemoActor" '{"op":"status"}')
if echo "$ALARM_RESP" | grep -q '"status":"ok"'; then
    echo -e "  ${GREEN}AlarmDemoActor OK${NC}"
else
    echo -e "  ${YELLOW}AlarmDemoActor not responsive (skipping alarm tests): $ALARM_RESP${NC}"
    ALARM_RESP=""
fi
...
if [ -n "$ALARM_RESP" ]; then
    ...
    for i in 1 2 3; do
        send_op "AlarmDemoActor" "{\"op\":\"enqueue\",\"data\":\"task-$i\"}" 5 >/dev/null
    done
    echo -e "  ${GREEN}Enqueued 3 tasks${NC}"

    echo "  Scheduling alarm (2s delay)..."
    START_RESP=$(send_op "AlarmDemoActor" '{"op":"start","delay_ms":2000}' 5)
    ...
    STATUS_RESP=$(send_op "AlarmDemoActor" '{"op":"status"}' 5)
    ...
    CANCEL_RESP=$(send_op "AlarmDemoActor" '{"op":"cancel"}' 5)
...
# Step 6: Batch Message Benchmark
...
MSG_BATCH_RESP=$(send_op "ChatRoomActor" "{\"op\":\"send_message_batch\",\"count\":$BATCH_MSGS,\"user_id\":\"bench-user\",\"content\":\"Benchmark message with realistic payload sizing for chat\"}" 120)
...
BATCH_RL_RESP=$(send_op "RateLimiterActor" "{\"op\":\"check_batch\",\"user_id\":\"bench\",\"count\":$BATCH_RATE_CHECKS}" 120)
...
CHAT_STATS=$(send_op "ChatRoomActor" '{"op":"stats"}')
RL_STATS=$(send_op "RateLimiterActor" '{"op":"status"}')
...
" 2>/dev/null || echo "  (Could not parse final stats)"

echo ""
echo "================================================================"
echo -e "  ${GREEN}Cloudflare Workers Guild Chat Test Complete (Python)${NC}"
echo "================================================================"
