...
async fn test_memoize_factory_roundtrip_via_service_locator() {
    ...
    // Miss
    assert!(matches!(
        facet.before_method("process", b"data", &HashMap::new()).await.unwrap(),
        InterceptResult::Continue
    ));

    // Store result
    facet.after_method("process", b"data", b"computed-result", &HashMap::new()).await.unwrap();

    // Hit
    let hit = facet.before_method("process", b"data", &HashMap::new()).await.unwrap();
...
async fn test_memoize_payload_only_not_message_id() {
    ...
    facet.on_attach("actor-payload-1", serde_json::json!({})).await.unwrap();

    let payload = b"stable-payload";
    facet.after_method("compute", payload, b"result-v1", &HashMap::new()).await.unwrap();

    // Same payload → hit regardless of conceptual "message ID"
    let hit = facet.before_method("compute", payload, &HashMap::new()).await.unwrap();
    assert!(
...
async fn test_memoize_tell_pattern_short_circuits_handler() {
    let kv: Arc<dyn KeyValueStore> = MemKv::new();
    let mut facet = make_facet(kv, 0, 0, vec![]);
    facet.on_attach("actor-tell-1", serde_json::json!({})).await.unwrap();

    facet.after_method("ingest", b"event-data", b"ok", &HashMap::new()).await.unwrap();

    let res = facet.before_method("ingest", b"event-data", &HashMap::new()).await.unwrap();
...
async fn test_memoize_handler_filter_integration() {
    ...
    facet.on_attach("actor-filter-1", serde_json::json!({})).await.unwrap();

    // Prime cache for both
    facet.after_method("allowed_handler", b"x", b"cached-allowed", &HashMap::new()).await.unwrap();
    facet.after_method("blocked_handler", b"x", b"cached-blocked", &HashMap::new()).await.unwrap();

    // allowed_handler: hit
    assert!(matches!(
        facet.before_method("allowed_handler", b"x", &HashMap::new()).await.unwrap(),
        InterceptResult::ShortCircuit(_)
    ));

    // blocked_handler: NOT intercepted → always miss
    assert!(matches!(
        facet.before_method("blocked_handler", b"x", &HashMap::new()).await.unwrap(),
        InterceptResult::Continue
    ));
}
...
async fn test_memoize_max_entries_eviction_integration() {
    ...
    facet.on_attach("actor-evict-1", serde_json::json!({})).await.unwrap();

    facet.after_method("op", b"a", b"va", &HashMap::new()).await.unwrap();
    facet.after_method("op", b"b", b"vb", &HashMap::new()).await.unwrap();
    facet.after_method("op", b"c", b"vc", &HashMap::new()).await.unwrap();

    // "a" should have been evicted
    assert!(matches!(
        facet.before_method("op", b"a", &HashMap::new()).await.unwrap(),
        InterceptResult::Continue
    ));

    // "b" and "c" should still be cached
    assert!(matches!(
        facet.before_method("op", b"b", &HashMap::new()).await.unwrap(),
        InterceptResult::ShortCircuit(_)
    ));
    assert!(matches!(
        facet.before_method("op", b"c", &HashMap::new()).await.unwrap(),
        InterceptResult::ShortCircuit(_)
    ));
}
