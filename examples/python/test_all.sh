#!/usr/bin/env bash
# Run all Python examples in order. Stops on first failure unless -c is given.
#
# Usage:
...
for name in "${EXAMPLES[@]}"; do
    ...
    start=$SECONDS
    bash "$test_script" $args >/tmp/test_all_py_out.txt 2>&1 &
    test_pid=$!
    spinner_chars='|/-\'
    spinner_idx=0
    while kill -0 "$test_pid" 2>/dev/null; do
        elapsed_now=$((SECONDS - start))
        last_line="$(tail -1 /tmp/test_all_py_out.txt 2>/dev/null | tr -d '\r\n' | cut -c1-60)"
    ...
    if [[ $test_rc -eq 0 ]]; then
        ...
        echo "    --- last 20 lines ---"
        tail -20 /tmp/test_all_py_out.txt | sed 's/^/    /'
        ((failed++)) || true
        printf 'LAST_EXAMPLE=%s\nLAST_STATUS=FAILED\n' "$name" > "$STATE_FILE"
