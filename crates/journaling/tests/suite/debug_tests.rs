...
mod sqlite_tests {
    ...
    async fn test_append_entry_called() {
        ...
        let before_result = facet
            .before_method("test", b"test", &std::collections::HashMap::new())
            .await;
    ...
    async fn test_sequence_after_replay() {
        ...
        facet2
            .before_method("test", b"new", &std::collections::HashMap::new())
            .await
            .unwrap();
        storage.flush().await.unwrap();
        ...
        for cycle in 1..=3 {
            ...
            facet
                .after_method(
                    "test",
                    &[],
                    &format!("result-{}", cycle).into_bytes(),
                    &std::collections::HashMap::new(),
                )
                .await
                .unwrap();
