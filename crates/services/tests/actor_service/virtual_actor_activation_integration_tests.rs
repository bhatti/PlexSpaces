...
use plexspaces_proto::actor::v1::{
    actor_service_server::ActorService as ActorServiceTrait, GetOrActivateActorRequest,
};
...
async fn test_virtual_actor_type_registration_and_lazy_activation() {
    ...
    let response =
        ActorServiceTrait::get_or_activate_actor(&*actor_service, Request::new(req)).await;
...
async fn test_virtual_actor_reactivation_type_registered_only() {
    ...
    request.metadata_mut().insert(
        "x-namespace",
        MetadataValue::from_static("orbit-read-state-ts"),
    );
