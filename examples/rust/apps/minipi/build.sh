...
OUTPUT_WASM="$SCRIPT_DIR/minipi_actor.wasm"
EMBEDDED_WASM="$SCRIPT_DIR/minipi_actor_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/minipi_wasm.wasm"
...
echo "Building MiniPi (Rust WASM)..."
...
for candidate in \
  ...
  "$REPO_ROOT/examples/typescript/apps/minipi/node_modules/@bytecodealliance/jco/lib/wasi_snapshot_preview1.reactor.wasm" \
