#!/bin/bash
# Test HPC Ensemble - EFlows4HPC style (Python WASM)
...
APP_ID="migrating-eflows4hpc-ensemble-py"
ACTOR_TYPE="ensemble"
NUM_TASKS=20
...
echo "================================================================"
echo "  EFlows4HPC → PlexSpaces: HPC Ensemble (Python WASM)"
...
DEPLOY_OUT=$(curl -s -w "\n%{http_code}" -X POST "http://localhost:$HTTP_PORT/api/v1/applications/deploy" \
  ...
  -F "name=migrating-eflows4hpc-ensemble-py" \
  -F "version=1.0.0" \
