...
WASM_FILE="$SCRIPT_DIR/batch_image_classification_actor.wasm"
...
APP_ID="batch-image-classification-rust"
APP_NAME="batch-image-classification-rust"
LEADER_ACTOR="leader"
TOTAL_IMAGES=2400000
WORKER_COUNT=16
BATCHES=24
IMAGE_VECTOR_SIZE=96
MODEL_ID="resnet50-simulated"
...
run_payload=$(cat <<EOF
{"op":"run","total_images":$TOTAL_IMAGES,"worker_count":$WORKER_COUNT,"batches":$BATCHES,"image_vector_size":$IMAGE_VECTOR_SIZE,"model_id":"$MODEL_ID"}
EOF
)
...
echo "Step 3: Metrics"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
export IMAGE_CLASSIFICATION_WALL_MS="$wall_ms"
export IMAGE_CLASSIFICATION_EXPECTED_NODES="${#NODE_LIST[@]}"
...
wall_ms = int(os.environ.get("IMAGE_CLASSIFICATION_WALL_MS", "0"))
...
classification_ops = int(payload.get("classification_operation_count", 0))
...
expected_nodes = int(os.environ.get("IMAGE_CLASSIFICATION_EXPECTED_NODES", "1"))

print("  Batch Image Classification (Rust WASM)")
print("  Data size")
print(
    "  total_images={images} worker_count={workers} batches={batches} image_vector_size={vector}".format(
        images=payload.get("total_images", 0),
        workers=payload.get("worker_count", 0),
        batches=payload.get("batches", 0),
        vector=payload.get("image_vector_size", 0),
    )
)
print("  Topology")
print(
    "  node_count={node_count} worker_node_count={worker_nodes} actor_count={actor_count} leader_node_id={leader} scatter_gather_rounds={rounds}".format(
...
print(f"  classification_operation_count={classification_ops}")
print(f"  images_processed={payload.get('images_processed', 0)}")
print("  Result")
...
print("  Predictions")
for label in sorted(predictions):
    print(f"  {label}={predictions[label]}")

print("  Per-role")
for role in sorted(roles):
    info = roles[role]
    print(
        "  {role}: actors={actors} messages={messages} class_ops={class_ops} images={images} compute_ms={compute_ms} coord_ms={coord_ms} avg_latency_ms={avg:.2f} max_latency_ms={max_latency} errors={errors}".format(
            role=role,
            actors=info.get("actors", 0),
            messages=info.get("messages", 0),
            class_ops=info.get("classification_operations", 0),
            images=info.get("images_processed", 0),
...
for node_id in sorted(nodes):
    ...
    print(
        "  {node_id} ({address}): actors={actors} leader_actors={leader_actors} worker_actors={worker_actors} messages={messages} leader_messages={leader_messages} worker_messages={worker_messages} class_ops={class_ops} images={images} compute_ms={compute_ms} coord_ms={coord_ms} avg_latency_ms={avg:.2f} max_latency_ms={max_latency} errors={errors}".format(
            node_id=node_id,
            address=address,
            actors=info.get("actors", 0),
...
if node_count < expected_nodes:
    raise SystemExit(
        f"expected at least {expected_nodes} nodes in metrics but saw {node_count}"
    )
...
if classification_ops <= 0:
    raise SystemExit("expected positive classification operation count")
if payload.get("images_processed", 0) <= 0:
    raise SystemExit("expected positive images_processed count")
if not predictions:
    raise SystemExit("expected aggregated prediction counts")
PY
  echo -e "${RED}Metrics validation failed${NC}"
  exit 1
}
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo -e "${GREEN}Batch image classification test passed.${NC}"
