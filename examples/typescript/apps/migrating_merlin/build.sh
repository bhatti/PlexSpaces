#!/usr/bin/env bash
# Build Merlin parameter sweep WASM actor (TypeScript)
set -euo pipefail
...
echo "Building TypeScript Merlin parameter sweep → WASM..."
echo ""
...
if [ ! -f "sweep_actor.js" ]; then
  echo "  ERROR: sweep_actor.js not produced"
  exit 1
fi
echo "  ✓ sweep_actor.js"
...
if ! $JCO componentize sweep_actor_bundle.mjs --wit "$WIT_DIR" -n actor-world -o "$OUTPUT_WASM" --disable all 2>&1; then
