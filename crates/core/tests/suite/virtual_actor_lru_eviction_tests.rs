...
use plexspaces_core::{VirtualActorManager, ActorRegistry, ActorId, RequestContext, MessageSender};
use plexspaces_core::virtual_actor_lifecycle_facet::VirtualActorLifecycleFacet;
...
impl VirtualActorLifecycleFacet for MockVirtualActorLifecycleFacet {
    ...
    async fn should_deactivate(&self, _idle_timeout: Duration) -> bool {
        false
    }
...
fn create_mock_facet(actor_id: ActorId) -> Arc<RwLock<Box<dyn VirtualActorLifecycleFacet>>> {
...
async fn test_lru_eviction_basic() {
    ...
    let object_registry = Arc::new(ObjectRegistryImpl::new());
    let actor_registry = Arc::new(ActorRegistry::new(
        object_registry as Arc<dyn plexspaces_core::ObjectRegistry>,
        "test-node".to_string(),
    ));
    ...
    let actor_ids: Vec<ActorId> = (0..4)
        .map(|i| format!("actor-{}@test-node", i))
        .collect();
    ...
    assert_eq!(instances.unwrap().len(), 4, "Should have 4 active instances");
    ...
    assert_eq!(evicted.len(), 2, "Should evict 2 actors to stay under limit");
    assert!(evicted.contains(&actor_ids[0]), "Should evict oldest actor");
    assert!(evicted.contains(&actor_ids[1]), "Should evict second oldest actor");
    ...
    assert_eq!(instances_after.len(), 3, "Should have 3 active instances after eviction");
    assert!(!instances_after.iter().any(|i| i.actor_id == actor_ids[0]), "Oldest should be evicted");
    assert!(!instances_after.iter().any(|i| i.actor_id == actor_ids[1]), "Second oldest should be evicted");
}
...
async fn test_lru_eviction_ordering() {
    ...
    assert_eq!(evicted[0], actor_id_1, "Should evict oldest actor (actor_1)");
    assert!(!evicted.contains(&actor_id_2), "Should not evict actor_2 (more recent)");
}
...
async fn test_lru_eviction_multiple_types() {
    ...
    assert!(!evicted_1.iter().any(|id| id.contains("type2")), "Type1 eviction shouldn't affect Type2");
    assert!(!evicted_2.iter().any(|id| id.contains("type1")), "Type2 eviction shouldn't affect Type1");
}
...
async fn test_update_last_access() {
    ...
    let initial_access = instances.iter()
    ...
    let active_instances_after = manager.registry().active_instances_by_type().read().await;
    let instances_after = active_instances_after.get(&actor_type).unwrap();
    let updated_access = instances_after.iter()
        .find(|i| i.actor_id == actor_id)
        .unwrap()
        .last_access;
    
    assert!(updated_access > initial_access, "last_access should be updated");
}
...
async fn test_remove_from_active_tracking() {
    ...
    assert!(instances_after.is_none() || instances_after.unwrap().is_empty(), 
...
async fn test_max_pool_per_actor_type_config() {
    ...
    assert_eq!(default_max, DEFAULT_MAX_POOL_PER_ACTOR_TYPE, "Should use default max_pool");
