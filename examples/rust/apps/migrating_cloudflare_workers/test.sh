...
# Test Guild Chat - Discord-style Real-Time Chat Server (Rust WASM)
#
...
APP_ID="rs-cloudflare-guild-chat"
NUM_USERS=5
BATCH_MSGS=200
...
for _attempt in 1 2 3; do
  DEPLOY_OUT=$(curl -s -w "\n%{http_code}" -X POST "http://$HTTP_HOST:$HTTP_PORT/api/v1/applications/deploy" \
      ...
      -F "name=rs-cloudflare-guild-chat" \
      -F "version=1.0.0" \
...
echo ""
echo "================================================================"
echo -e "  ${GREEN}Cloudflare Workers Guild Chat Test Complete (Rust)${NC}"
echo "================================================================"
