...
| Function | Description | Example |
|----------|-------------|---------|
| `host.ts_write(tuple_json)` | Write tuple (JSON array) | `host.ts_write(json.dumps(["task", "worker-1", 123]))` |
| `host.ts_read(pattern_json)` | Read tuple (non-destructive, wildcards: null or "*") | `tuple = json.loads(host.ts_read(json.dumps(["task", "*", None])))` |
| `host.ts_take(pattern_json)` | Take tuple (destructive read) | `tuple = json.loads(host.ts_take(json.dumps(["task", "*", None])))` |
| `host.ts_read_all(pattern_json)` | Read all matching tuples | `tuples = json.loads(host.ts_read_all(json.dumps(["task", "*", None])))` |
...
| `host.process_groups.broadcast(group, msg_type, payload)` | Broadcast to all group members | `host.process_groups.broadcast("chat-room", "message", {"text": "hello"})` |
...
| **TupleSpace** | ❌ Not available | ✅ `host.ts_*` | ✅ Extra |
