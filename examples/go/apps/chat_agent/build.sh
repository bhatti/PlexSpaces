...
echo "Building ChatAgent (Go WASM)..."
...
go mod tidy
tinygo build -target=wasi -o chat_agent_actor_core.wasm .
wasm-tools component embed "$WIT_DIR" -w actor-world chat_agent_actor_core.wasm -o chat_agent_actor_embedded.wasm
wasm-tools component new chat_agent_actor_embedded.wasm --adapt "wasi_snapshot_preview1=$ADAPTER" -o "$OUTPUT_WASM"
rm -f chat_agent_actor_core.wasm chat_agent_actor_embedded.wasm
