#!/usr/bin/env bash
# Build Order Fulfillment Workflow WASM actor: TypeScript → JS → ESM bundle → WASM component
set -euo pipefail
...
echo "Building TypeScript order_fulfillment (Workflow) → WASM..."
echo ""
...
if [ ! -f "order_fulfillment_actor.js" ]; then
  echo "  ERROR: order_fulfillment_actor.js not produced"
  exit 1
fi
echo "  ✓ order_fulfillment_actor.js"
...
if ! $JCO componentize order_fulfillment_actor_bundle.mjs --wit "$WIT_DIR" -n actor-world -o "$OUTPUT_WASM" --disable all 2>&1; then
