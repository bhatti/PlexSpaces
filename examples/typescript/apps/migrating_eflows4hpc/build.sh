#!/usr/bin/env bash
# Build EFlows4HPC Ensemble WASM actor (TypeScript)
set -euo pipefail
...
echo "Building TypeScript EFlows4HPC Ensemble → WASM..."
echo ""
...
if [ ! -f "ensemble_actor.js" ]; then
  echo "  ERROR: ensemble_actor.js not produced"
  exit 1
fi
echo "  ✓ ensemble_actor.js"
...
if ! $JCO componentize ensemble_actor_bundle.mjs --wit "$WIT_DIR" -n actor-world -o "$OUTPUT_WASM" --disable all 2>&1; then
