...
CHAT_RESP=$(send_op "$CHAT_ACTOR" '{"op":"stats"}')
...
RL_RESP=$(send_op "$RL_ACTOR" '{"op":"stats"}')
...
for i in $(seq 1 $NUM_USERS); do
    RESP=$(send_op "$CHAT_ACTOR" "{\"op\":\"join\",\"user_id\":\"user-$i\"}" 5)
...
for i in $(seq 1 $NUM_USERS); do
    RESP=$(send_op "$CHAT_ACTOR" "{\"op\":\"send_message\",\"user_id\":\"user-$i\",\"content\":\"Hello from user-$i!\"}" 5)
...
MEMBERS_RESP=$(send_op "$CHAT_ACTOR" '{"op":"get_members"}' 5)
...
HIST_RESP=$(send_op "$CHAT_ACTOR" '{"op":"get_history","limit":10}' 5)
...
LEAVE_RESP=$(send_op "$CHAT_ACTOR" '{"op":"leave","user_id":"user-1"}' 5)
...
DENIED=0
for i in $(seq 1 7); do
    RESP=$(send_op "$RL_ACTOR" '{"op":"check_rate","user_id":"test-user"}' 5)
...
MSG_BATCH_RESP=$(send_op "$CHAT_ACTOR" "{\"op\":\"send_message_batch\",\"count\":$BATCH_MSGS,\"user_id\":\"bench-user\",\"content\":\"Benchmark message with realistic payload sizing for chat\"}" 120)
...
BATCH_RL_RESP=$(send_op "$RL_ACTOR" "{\"op\":\"check_rate_batch\",\"user_id\":\"bench\",\"count\":$BATCH_RATE_CHECKS}" 120)
...
CHAT_STATS=$(send_op "$CHAT_ACTOR" '{"op":"stats"}')
RL_STATS=$(send_op "$RL_ACTOR" '{"op":"stats"}')
...
echo ""
echo "================================================================"
echo -e "  ${GREEN}Cloudflare Workers Guild Chat (TypeScript) Test Complete${NC}"
echo "================================================================"
