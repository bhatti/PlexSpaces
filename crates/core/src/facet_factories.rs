...
use crate::{RequestContext, ProcessGroupService, ServiceLocator};
...
impl FacetFactory for KeyValueFacetFactory {
    async fn create(&self, config: Value) -> Result<Box<dyn Facet>, FacetError> {
        use plexspaces_facet::capabilities::keyvalue::{KeyValueFacet, KEYVALUE_FACET_DEFAULT_PRIORITY};
...
impl FacetFactory for HttpClientFacetFactory {
    async fn create(&self, config: Value) -> Result<Box<dyn Facet>, FacetError> {
        use plexspaces_facet::capabilities::http_client::{HttpClientFacet, HTTP_CLIENT_FACET_DEFAULT_PRIORITY};
    ...
    fn metadata(&self) -> FacetMetadata {
        FacetMetadata {
            ...
            attached_at: std::time::Instant::now(),
            config: serde_json::Value::Null,
            priority: plexspaces_facet::capabilities::http_client::HTTP_CLIENT_FACET_DEFAULT_PRIORITY,
        }
    }
}
...
mod tests {
    use super::*;
    use crate::{LockManager, JournalStorage, KeyValueStore, ServiceLocator};
    ...
    async fn create_test_service_locator() -> Arc<dyn ServiceLocator> {
        ...
        if service_locator.get_object_registry().await.is_none() {
            ...
            service_locator.register_object_registry(object_registry).await;
        }
        ...
        if service_locator.get_process_group_service().await.is_none() {
            ...
            service_locator.register_process_group_service(process_group_service).await;
        }
    ...
    async fn test_factories_use_default_priority() {
        ...
        let config = serde_json::json!({});
        let facet = factory.create(config).await.unwrap();
        assert_eq!(facet.get_priority(), plexspaces_facet::capabilities::locks::LOCK_FACET_DEFAULT_PRIORITY);
        
        let factory = RegistryFacetFactory::new(service_locator.clone());
        let config = serde_json::json!({});
        let facet = factory.create(config).await.unwrap();
        assert_eq!(facet.get_priority(), plexspaces_facet::capabilities::registry::REGISTRY_FACET_DEFAULT_PRIORITY);
        
        let factory = ProcessGroupFacetFactory::new(service_locator);
        let config = serde_json::json!({});
        let facet = factory.create(config).await.unwrap();
        assert_eq!(facet.get_priority(), plexspaces_facet::capabilities::process_groups::PROCESS_GROUP_FACET_DEFAULT_PRIORITY);
    }
}
