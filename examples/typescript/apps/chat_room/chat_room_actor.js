...
function actorInstanceName(actorId) {
    ...
    if (slashIdx >= 0)
        return actorId.slice(0, slashIdx);
    return actorId;
}
/**
...
// ─────────────────────────────────────────────────────────────────────────────
const VALID_CONNECTION_TRANSITIONS = {
 ...
 */
class SessionActor extends PlexSpacesActor {
    getDefaultState() {
        return {
            actor_id: "",
            application_id: "",
    ...
    onSend_channel_message(payload) {
        ...
        return host.ask(channelActorId(this.state.guild_id, channelId), "post_message", {
            user_id: this.state.user_id,
            session_id: this.state.session_id,
            text,
        }, 5000);
    ...
    onSet_typing(payload) {
        ...
        return host.ask(channelActorId(this.state.guild_id, channelId), "mark_typing", { user_id: this.state.user_id, ttl_ms: ttlMs }, 5000);
    }
    ...
    onRead_channel(payload) {
        ...
        unread[channelId] = 0;
        this.state.unread_by_channel = unread;
        host.ask(connectionFsmActorId(this.state.session_id), "transition", { to: "idle" }, 5000);
        return {
            status: "read",
            channel_id: channelId,
            session_id: this.state.session_id,
 ...
 */
class ConnectionFSM extends PlexSpacesActor {
    getDefaultState() {
        return {
            actor_id: "",
            application_id: "",
    ...
    onTransition(payload) {
        ...
        const allowed = VALID_CONNECTION_TRANSITIONS[this.state.fsm_state] ?? new Set();
        if (!allowed.has(to)) {
            return {
                status: "ignored",
 ...
 */
class GuildActor extends PlexSpacesActor {
    getDefaultState() {
        return {
            actor_id: "",
            application_id: "",
 ...
 */
class ChannelActor extends PlexSpacesActor {
    getDefaultState() {
        return {
            actor_id: "",
            application_id: "",
 ...
 */
class MessageStoreActor extends PlexSpacesActor {
    getDefaultState() {
        return {
            actor_id: "",
            application_id: "",
    ...
    onAppend_message(payload) {
        ...
        this.state.next_message_seq = Math.max(this.state.next_message_seq + 1, this.state.messages.length + 1);
 ...
 */
class FanoutActor extends PlexSpacesActor {
    getDefaultState() {
        return {
            actor_id: "",
            application_id: "",
 ...
 */
class AuditEventActor extends PlexSpacesActor {
    getDefaultState() {
        return {
            actor_id: "",
            application_id: "",
 ...
 */
class ModerationWorkflow extends WorkflowActor {
    getDefaultState() {
        return {
            actor_id: "",
            application_id: "",
