...
impl MemoryLockManager {
    ...
    fn lock_to_proto(lock_key: String, entry: &LockEntry) -> Lock {
        Lock {
            ...
            expires_at: Some(plexspaces_proto::prost_types::Timestamp::from(entry.expires_at)),
            lease_duration_secs: entry.lease_duration_secs,
            last_heartbeat: Some(plexspaces_proto::prost_types::Timestamp::from(entry.last_heartbeat)),
...
impl LockManager for MemoryLockManager {
    ...
    async fn acquire_lock(&self, ctx: &RequestContext, options: AcquireLockOptions) -> LockResult<Lock> {
        ...
        let key = (tenant_id.clone(), namespace.clone(), options.lock_key.clone());
