#!/bin/bash
# Test parameter sweep - Merlin style (Go WASM), worker pool with elastic size
...
APP_ID="migrating-merlin-sweep-go"
ACTOR_TYPE="sweep"
NUM_PARAMS=80
BATCH_SWEEPS=10
...
for _attempt in 1 2 3; do
  DEPLOY_OUT=$(curl -s -w "\n%{http_code}" -X POST "http://localhost:$HTTP_PORT/api/v1/applications/deploy" \
    ...
    -F "name=migrating-merlin-sweep-go" \
    -F "version=1.0.0" \
...
echo "================================================================"
echo -e "  ${GREEN}Merlin parameter sweep (Go) test complete${NC}"
echo "================================================================"
