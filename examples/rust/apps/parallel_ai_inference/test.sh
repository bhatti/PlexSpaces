...
WORKER_ACTOR_PATH="inference_worker:parallel_ai_inference_wasm"
BENCHMARK_ACTOR_PATH="benchmark:parallel_ai_inference_wasm"
ORCHESTRATOR_ACTOR_PATH="orchestrator:parallel_ai_inference_wasm"
...
cleanup() {
  ...
  for node in "${NODE_LIST[@]}"; do
    ...
    curl -s -X DELETE "http://${h}:${p}/api/v1/applications/$OTHER_APP_ID" >/dev/null 2>&1 || true
  done
}
trap cleanup EXIT
...
PY
  then
    echo "FAIL [$label]: response did not come from expected application namespace '$APP_ID'"
    echo "  Response: $response"
    exit 1
  fi
}
...
for node in "${NODE_LIST[@]}"; do
  ...
  if [ "$http_code" = "000" ]; then
    echo "Cannot connect to node at ${h}:${p}"
    exit 1
  fi
done
...
for node in "${NODE_LIST[@]}"; do
  ...
  if [ "$http_code" != "200" ] || ! echo "$body" | grep -qE '"success"[[:space:]]*:[[:space:]]*true'; then
    echo "Deploy failed on ${h}:${p}: $body"
    exit 1
  fi
done
rm -f "$TEMP_CONFIG"
...
require_nonempty "direct_infer" "$INFER_RESPONSE"
require_actor_namespace "direct_infer" "$INFER_RESPONSE"
require_field "direct_infer" "$INFER_RESPONSE" "status"
...
require_field "get_metrics" "$METRICS_RESPONSE" "requests_processed"

echo "Step 4: Test shard benchmark"
...
require_field "shard_benchmark" "$SHARD_BENCH_RESPONSE" "benchmark"
require_field "shard_benchmark" "$SHARD_BENCH_RESPONSE" "results"
...
require_field "scaling_benchmark" "$SCALING_RESPONSE" "results"
...
require_field "weak_scaling_benchmark" "$WEAK_RESPONSE" "results"
...
require_field "collective_benchmark" "$COLLECTIVE_RESPONSE" "benchmark"
require_field "collective_benchmark" "$COLLECTIVE_RESPONSE" "timings"
...
require_field "orchestrator_shard" "$ORCH_SHARD_RESPONSE" "status"
require_field "orchestrator_shard" "$ORCH_SHARD_RESPONSE" "total_processed"
...
require_field "orchestrator_collective" "$ORCH_COLLECTIVE_RESPONSE" "status"
require_field "orchestrator_collective" "$ORCH_COLLECTIVE_RESPONSE" "timings"
...
require_field "orchestrator_query" "$STATUS_RESPONSE" "mode"
require_field "orchestrator_query" "$STATUS_RESPONSE" "total_processed"
...
echo "Step 11: Retrieve benchmark result history"
...
require_field "get_results" "$ALL_RESULTS" "results"
require_field "get_results" "$ALL_RESULTS" "total_benchmarks"

echo "Rust parallel AI inference example passed."
