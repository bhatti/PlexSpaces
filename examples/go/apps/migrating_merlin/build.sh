#!/usr/bin/env bash
# Build Merlin parameter sweep WASM component (Go/TinyGo)
set -euo pipefail
...
echo "Building Merlin parameter sweep (Go)..."
...
go mod tidy
tinygo build -target=wasi -o sweep_actor_core.wasm .
wasm-tools component embed "$WIT_DIR" -w actor-world sweep_actor_core.wasm -o sweep_actor_embedded.wasm
wasm-tools component new sweep_actor_embedded.wasm --adapt "wasi_snapshot_preview1=$ADAPTER" -o "$OUTPUT_WASM"
rm -f sweep_actor_core.wasm sweep_actor_embedded.wasm
