...
async fn validate_mtls_config(mtls: &MtlsConfig) -> Result<(), SecurityValidationError> {
    ...
    let server_cert_path = resolve_path(&mtls.server_certificate_path, "PLEXSPACES_MTLS_SERVER_CERT")?;
    let server_key_path = resolve_path(&mtls.server_key_path, "PLEXSPACES_MTLS_SERVER_KEY")?;
    
    // Check CA certificate
    if ca_cert_path.is_empty() {
        return Err(SecurityValidationError::MissingCaCertificate("path is empty".to_string()));
    }
    ...
    if server_cert_path.is_empty() {
        return Err(SecurityValidationError::MissingServerCertificate("path is empty".to_string()));
    }
    if !Path::new(&server_cert_path).exists() {
        return Err(SecurityValidationError::MissingServerCertificate(server_cert_path));
    }
    
    // Check server key
    if server_key_path.is_empty() {
        return Err(SecurityValidationError::MissingServerKey("path is empty".to_string()));
    }
...
async fn generate_mtls_certificates(mtls: &MtlsConfig) -> Result<(), SecurityValidationError> {
    ...
    fs::create_dir_all(&cert_dir).map_err(|e| {
        SecurityValidationError::CertificateDirError(cert_dir.clone(), e)
    })?;
    ...
    fs::write(&ca_cert_path, ca_cert.cert_pem.as_bytes()).map_err(|e| {
        SecurityValidationError::CertificateReadError(ca_cert_path.clone(), e)
    })?;
    fs::write(&ca_key_path, ca_cert.key_pem.as_bytes()).map_err(|e| {
        SecurityValidationError::CertificateReadError(ca_key_path.clone(), e)
    })?;
    
    // Generate server certificate signed by CA
    let server_cert = generate_server_certificate(&ca_cert)?;
    fs::write(&server_cert_path, server_cert.cert_pem.as_bytes()).map_err(|e| {
        SecurityValidationError::CertificateReadError(server_cert_path.clone(), e)
    })?;
    fs::write(&server_key_path, server_cert.key_pem.as_bytes()).map_err(|e| {
        SecurityValidationError::CertificateReadError(server_key_path.clone(), e)
    })?;
...
fn generate_ca_certificate() -> Result<CertificatePair, SecurityValidationError> {
    ...
    let mut params = CertificateParams::new(vec![])
        .map_err(|e| SecurityValidationError::AutoGenerationFailed(
            format!("Failed to create CA certificate parameters: {}", e)
        ))?;
    params.distinguished_name = DistinguishedName::new();
    params.distinguished_name.push(DnType::CommonName, "PlexSpaces CA");
    params.distinguished_name.push(DnType::OrganizationName, "PlexSpaces");
    ...
    let key_pair = rcgen::KeyPair::generate()
        .map_err(|e| SecurityValidationError::AutoGenerationFailed(
            format!("Failed to generate CA key pair: {}", e)
        ))?;
    
    // Generate self-signed CA certificate
    let cert = params.self_signed(&key_pair)
        .map_err(|e| SecurityValidationError::AutoGenerationFailed(
            format!("Failed to generate CA certificate: {}", e)
        ))?;
...
fn generate_server_certificate(ca_cert: &CertificatePair) -> Result<CertificatePair, SecurityValidationError> {
    ...
    let ca_key_pair = KeyPair::from_pem(&ca_cert.key_pem)
        .map_err(|e| SecurityValidationError::AutoGenerationFailed(
    ...
    let mut params = CertificateParams::new(vec!["localhost".to_string(), "127.0.0.1".to_string()])
        .map_err(|e| SecurityValidationError::AutoGenerationFailed(
            format!("Failed to create server certificate parameters: {}", e)
        ))?;
    params.distinguished_name = DistinguishedName::new();
    params.distinguished_name.push(DnType::CommonName, "PlexSpaces Server");
    params.distinguished_name.push(DnType::OrganizationName, "PlexSpaces");
    ...
    let server_key_pair = KeyPair::generate()
        .map_err(|e| SecurityValidationError::AutoGenerationFailed(
            format!("Failed to generate server key pair: {}", e)
        ))?;
    ...
    let server_cert = params.signed_by(&server_key_pair, ca_cert_obj, &ca_key_pair)
        .map_err(|e| SecurityValidationError::AutoGenerationFailed(
            format!("Failed to sign server certificate: {}", e)
        ))?;
...
mod tests {
    ...
    async fn test_validate_jwt_config_missing_secret() {
        ...
        assert!(matches!(result.unwrap_err(), SecurityValidationError::MissingJwtSecret));
    }
    ...
    async fn test_validate_mtls_config_auto_generate() {
        ...
        assert!(server_key_content.contains("BEGIN PRIVATE KEY") || server_key_content.contains("BEGIN EC PRIVATE KEY"));
    }
    ...
    async fn test_generate_server_certificate() {
        ...
        assert!(server_cert.key_pem.contains("BEGIN") && server_cert.key_pem.contains("PRIVATE KEY"));
    }
}
