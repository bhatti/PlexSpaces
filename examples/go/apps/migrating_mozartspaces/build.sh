#!/usr/bin/env bash
# Build MozartSpaces Auction WASM component (Go/TinyGo)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../../../" && pwd)"
WIT_DIR="$REPO_ROOT/wit/plexspaces-actor"
OUTPUT_WASM="$SCRIPT_DIR/auction_actor.wasm"

cd "$SCRIPT_DIR"

echo "Building MozartSpaces Auction (Go)..."
...
tinygo build -target=wasi -o auction_actor_core.wasm .
wasm-tools component embed "$WIT_DIR" -w actor-world auction_actor_core.wasm -o auction_actor_embedded.wasm
wasm-tools component new auction_actor_embedded.wasm --adapt "wasi_snapshot_preview1=$ADAPTER" -o "$OUTPUT_WASM"
rm -f auction_actor_core.wasm auction_actor_embedded.wasm
