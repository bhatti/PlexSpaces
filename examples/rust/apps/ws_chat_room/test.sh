...
#
# Integration test for rust ws_chat_room.
#
# Two Node.js WsThinClient sessions (alice, bob) join the same room,
# alice sends a message, bob asserts receipt via WebSocket push.
#
# Usage:
#   bash test.sh [port]
#   KEEP_DEPLOYED=1 bash test.sh   # skip undeploy so the app stays accessible
#
...
WASM_FILE="$SCRIPT_DIR/ws_chat_room_actor.wasm"
CONFIG_FILE="$SCRIPT_DIR/app-config.toml"

WS_PORT="${1:-${WS_PORT:-8095}}"
WS_URL="${WS_URL:-ws://localhost:${WS_PORT}/ws}"
HTTP_URL="${HTTP_URL:-http://localhost:${WS_PORT}}"
APP_ID="rust-ws-chat-room"
...
# ─── 0. Build ─────────────────────────────────────────────────────────────────
log "Building ws_chat_room (Rust WASM)…"
cd "$SCRIPT_DIR"
bash build.sh
...
[ "$HTTP_CODE" = "200" ] || fail "Node not reachable (HTTP $HTTP_CODE). Start with: ./scripts/server.sh $WS_PORT"
...
# ─── 4. Integration test via WsThinClient ─────────────────────────────────────
log "Running Node.js WebSocket integration test…"
...
const APP_NS         = "${APP_ID}";
const ROOM           = "rust-lobby-" + WsThinClient.newUlid().slice(-6);
...
async function runTest() {
  ...
  // Bob registers a listener for the pushed chat_message before anyone joins.
  ...
  // Step A: alice joins
  const aliceJoin = await alice.ask(chatRoomId, "join", { actor_id: aliceActor, username: "alice" }, 10_000);
  if (!aliceJoin.success) throw new Error("alice join failed: " + JSON.stringify(aliceJoin));

  // Step B: bob joins
  const bobJoin = await bob.ask(chatRoomId, "join", { actor_id: bobActor, username: "bob" }, 10_000);
  if (!bobJoin.success) throw new Error("bob join failed: " + JSON.stringify(bobJoin));

  // Step C: alice sends a message (cast — fire-and-forget)
  const testText = "hello from rust alice " + Date.now();
  await alice.tell(chatRoomId, "send", { sender_actor_id: aliceActor, text: testText });

  // Step D: bob receives the pushed chat_message
  const received = await messagePromise;
  if (received.text !== testText) {
    throw new Error("Expected text '" + testText + "', got '" + JSON.stringify(received) + "'");
  }

  // Step E: check status
  const status = await alice.ask(chatRoomId, "status", {}, 5_000);
  if (status.member_count !== 2) {
    throw new Error("Expected 2 members, got " + status.member_count);
  }
  ...
  console.log("[test] PASS: alice → ChatRoomActor PG broadcast → bob message delivery confirmed (Rust)");
}
