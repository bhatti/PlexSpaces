...
// ../../../../sdks/typescript/dist/workflow.js
function effectiveMaxAttempts(c) {
  const n = c.max_attempts ?? 0;
  return n === 0 ? 1 : n;
}
function withRetry(fn, config = {}) {
  const fromConfig = config.max_attempts !== void 0 ? effectiveMaxAttempts(config) : 0;
...
// document_processing_actor.ts
var MAX_RETRIES_PER_STEP = 3;
var OCR_PAGES = 4;
var OCR_MS_PER_PAGE = 20;
var CLASSIFY_MS = 30;
var EXTRACT_MS = 25;
var STORE_MS = 15;
function simulateTransientFailure(jobId, attemptNumber) {
  if (attemptNumber > 1) return false;
  let h = 0;
  for (let i = 0; i < jobId.length; i++) h = h * 31 + jobId.charCodeAt(i) >>> 0;
  return h % 3 === 0;
}
var DocumentProcessingActor = class extends WorkflowActor {
  getDefaultState() {
    return {
      job_id: "",
      status: "pending",
  ...
  run(payload) {
    ...
    if (this.state.status !== "pending" && this.state.status !== "ocr_done" && this.state.status !== "classified" && this.state.status !== "extracted") {
      return {
  ...
  finishFailed(computeMs, t0, failed_step, error_message) {
    this.state.status = "failed";
    this.state.updated_at_ms = host.nowMs();
    const totalElapsedMs = host.nowMs() - t0;
    const computeReported = Math.min(computeMs, totalElapsedMs);
...
var actorInstance = new DocumentProcessingActor();
var actor2 = {
