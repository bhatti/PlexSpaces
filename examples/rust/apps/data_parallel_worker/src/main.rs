...
use plexspaces_sdk::{NodeBuilder, ShardGroupClientLocal, ShardGroupClientTrait};
use serde_json::json;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    behavior_registry
        .register_simple("worker", || {
            Box::pin(async move {
                Ok(Box::new(WorkerActor::new("worker".to_string()))
    ...
    // Create ShardGroup client from service locator
    let mut client = ShardGroupClientLocal::new(service_locator.clone()).await?;
    ...
    info!(
        "Creating ShardGroup 'worker-pool-1' with {} workers...",
        WORKER_COUNT
    );
    ...
    let pool_id = group
        .config
        .as_ref()
        .map(|config| config.group_id.clone())
        .unwrap_or_default();
    let create_duration = create_start.elapsed();
    info!(
        "✅ Created ShardGroup: {} (took {:?})",
        pool_id, create_duration
    );
    ...
    for i in 0..TOTAL_MESSAGES {
        ...
        updates.insert(
            key.clone(),
            json!({ "action": "set", "key": key, "value": i }),
        );
    }
    ...
    let updates_sent = bulk_resp.updates_sent as u64;
    let updates_succeeded = bulk_resp.updates_succeeded as u64;
    let updates_failed = bulk_resp.updates_failed as u64;
    info!("  Messages sent: {}", updates_sent);
    info!("  Messages succeeded: {}", updates_succeeded);
    info!("  Messages failed: {}", updates_failed);
    info!(
        "  Throughput: {:.2} msg/s",
        TOTAL_MESSAGES as f64 / update_duration.as_secs_f64()
    );
    info!(
        "  Avg latency per message: {:.2} ms",
        update_duration.as_millis() as f64 / TOTAL_MESSAGES as f64
    );

    // Benchmark: map (query all workers)
    info!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    info!("Benchmark: Map (query all {} workers)", WORKER_COUNT);
    info!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    ...
    let map_resp = client.map(pool_id.clone(), map_task.clone()).await?;
    let map_results: Vec<serde_json::Value> = map_resp
        .shard_results
        .into_iter()
        .filter(|result| result.success)
        .filter_map(|result| result.response)
        .filter_map(|response| serde_json::from_slice(&response.payload).ok())
        .collect();
    ...
    info!(
        "  Avg latency per worker: {:.2} ms",
        map_duration.as_millis() as f64 / WORKER_COUNT as f64
    );

    // Benchmark: scatter-gather (aggregate stats)
    info!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    info!(
        "Benchmark: Scatter-Gather (aggregate from {} workers)",
        WORKER_COUNT
    );
    info!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

    let reduce_start = Instant::now();
    let scatter_resp = client
        .scatter_gather(
            pool_id.clone(),
            json!({ "action": "stats" }),
        ...
        .await?;
    let reduce_result = scatter_resp
        .result
        .and_then(|result| serde_json::from_slice(&result.payload).ok())
        .unwrap_or_else(|| json!({ "error": "No results from workers" }));
    let reduce_duration = reduce_start.elapsed();

    info!("✅ Scatter-gather completed:");
    ...
    if let Some(ref accessor) = metrics_accessor {
        ...
        if let Some(ref initial) = initial_metrics {
            let messages_routed = final_metrics
                .messages_routed
                .saturating_sub(initial.messages_routed);
            let local_deliveries = final_metrics
                .local_deliveries
                .saturating_sub(initial.local_deliveries);
            let remote_deliveries = final_metrics
                .remote_deliveries
                .saturating_sub(initial.remote_deliveries);
            let failed_deliveries = final_metrics
                .failed_deliveries
                .saturating_sub(initial.failed_deliveries);
        ...
        info!(
            "  Shard operations: {}",
            final_metrics.shard_operations_total
        );
        info!(
            "  Shard operations failed: {}",
            final_metrics.shard_operations_failed
        );
    }
    ...
    info!(
        "  Map: {:?} ({:.2} workers/s)",
        map_duration,
        WORKER_COUNT as f64 / map_duration.as_secs_f64()
    );
    info!("  Scatter-gather: {:?}", reduce_duration);
    info!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
