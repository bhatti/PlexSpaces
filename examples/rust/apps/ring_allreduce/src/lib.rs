    ...
    fn handle(from_actor: String, msg_type: String, payload: Vec<u8>) -> Result<Vec<u8>, String> {
        ...
        match (role.as_str(), op.as_str()) {
            ...
            ("worker", "ring_step") => worker::WorkerActor::default()
                .handle_operation(&from_actor, "ring_step", &payload),
            ("worker", "finalize_round") => worker::WorkerActor::default()
                .handle_operation(&from_actor, "finalize_round", &payload),
...
mod tests {
    ...
    fn actor_name_from_actor_id_handles_canonical_ids() {
        assert_eq!(
            actor_name_from_actor_id(
                "worker//ring_allreduce_wasm::ring-allreduce-rust@test-node-8093"
            ),
    ...
    fn resolve_role_prefers_declared_fields() {
        let config = InitConfig {
            actor_id: Some(
                "ring-allreduce-123//ring_allreduce_wasm::ring-allreduce-rust@test-node-8093"
                    .to_string(),
            ),
