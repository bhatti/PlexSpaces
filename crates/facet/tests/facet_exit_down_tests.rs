...
use plexspaces_facet::{ExitReason, Facet, FacetError};
...
impl TestExitDownFacet {
    fn new() -> (
        Self,
        std::sync::Arc<tokio::sync::RwLock<Vec<(String, String, ExitReason)>>>,
        std::sync::Arc<tokio::sync::RwLock<Vec<(String, String, ExitReason)>>>,
    ) {
...
async fn test_facet_on_down_called() {
    ...
    // Call on_down
    let result = facet
        .on_down(monitoring_actor_id, monitored_actor_id, &reason)
        .await;
    ...
    assert_eq!(
        calls[0].0, monitoring_actor_id,
        "monitoring_actor_id should match"
    );
    assert_eq!(
        calls[0].1, monitored_actor_id,
        "monitored_actor_id should match"
    );
    assert_eq!(calls[0].2, reason, "reason should match");
}
...
async fn test_facet_on_exit_with_different_reasons() {
    ...
    // Test Normal
    facet
        .on_exit(actor_id, from_actor_id, &ExitReason::Normal)
        .await
        .unwrap();

    // Test Shutdown
    facet
        .on_exit(actor_id, from_actor_id, &ExitReason::Shutdown)
        .await
        .unwrap();

    // Test Killed
    facet
        .on_exit(actor_id, from_actor_id, &ExitReason::Killed)
        .await
        .unwrap();
