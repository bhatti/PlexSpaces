...
use super::test_helpers::{find_actor_helper, unregister_actor_helper};
...
async fn wait_for_actor_state(
    ...
    let check_complete = async {
        ...
        loop {
            ...
            let is_dead =
                result.is_err() || matches!(result, Ok(plexspaces_node::ActorLocation::Remote(_)));
            ...
            if iterations >= max_iterations {
                return Err(format!(
                    "Max iterations reached waiting for actor {} to be {}",
                    actor_id,
                    if should_be_dead { "dead" } else { "alive" }
                ));
            }
    ...
    .await
    {
        ...
        Ok(Err(e)) => Err(e),
        Err(_) => Err(format!(
            "Timeout waiting for actor {} to be {}",
            actor_id,
            if should_be_dead { "dead" } else { "alive" }
        )),
    }
...
async fn test_exit_condition_cascading() {
    ...
    // Test 1: Error exit cascades to linked actor
    {
        ...
        assert!(
            result1_before.is_ok(),
            "actor1 should be alive before termination"
        );
        assert!(
            result2_before.is_ok(),
            "actor2 should be alive before termination"
        );

        terminate_actor(
            &node,
            "exit-test-1",
            ExitReason::Error("panic: test".to_string()),
        )
        .await;

        wait_for_actor_state(&node, "exit-test-2", true, 10)
            .await
    ...
    // Test 2: Normal exit does NOT cascade to linked actor
    {
        ...
        wait_for_actor_state(&node, "normal-test-2", false, 1)
            .await
    ...
    // Test 3: Chain cascading (actor1 -> actor2 -> actor3)
    {
        ...
        assert!(
            result3_before.is_ok(),
            "actor3 should be alive before termination"
        );
    ...
    // Test 4: Multiple links with partial unlinking
    {
        ...
        wait_for_actor_state(&node, "multi-2", false, 1)
            .await
            .expect("actor2 should be alive (unlinked)");
        wait_for_actor_state(&node, "multi-3", true, 2)
            .await
