...
impl DataParallelWorkerActor {
    fn initialize(config: &[u8]) -> Result<Vec<u8>, String> {
        let config: InitConfig = serde_json::from_slice(config)
    ...
    fn handle(from_actor: String, msg_type: String, payload: Vec<u8>) -> Result<Vec<u8>, String> {
        ...
        match (role.as_str(), op.as_str()) {
            ...
            ("worker", "process_shard_work") => worker::WorkerActor::default()
                .handle_operation(&from_actor, "process_shard_work", &payload)
                .or_else(|err| Ok(json_error(err))),
            _ => Ok(json_bytes(serde_json::json!({
