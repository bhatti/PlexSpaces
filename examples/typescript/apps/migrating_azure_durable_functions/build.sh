#!/usr/bin/env bash
# Build Document Processing Workflow WASM actor (TypeScript) - Azure Durable Functions style
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
WIT_DIR="$REPO_ROOT/wit/plexspaces-actor"
OUTPUT_WASM="$SCRIPT_DIR/document_processing_actor.wasm"

cd "$SCRIPT_DIR"

echo "Building TypeScript document_processing (Workflow) → WASM..."
echo ""
...
if [ ! -f "document_processing_actor.js" ]; then
  echo "  ERROR: document_processing_actor.js not produced"
  exit 1
fi
echo "  ✓ document_processing_actor.js"
...
if ! $JCO componentize document_processing_actor_bundle.mjs --wit "$WIT_DIR" -n actor-world -o "$OUTPUT_WASM" --disable all 2>&1; then
