...
OUTPUT_WASM="$SCRIPT_DIR/calculator_actor.wasm"
EMBEDDED_WASM="$SCRIPT_DIR/calculator_actor_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/calculator_wasm.wasm"
...
echo "Building calculator (Rust WASM, SDK+WIT)..."
