...
fn with_state<T>(f: impl FnOnce(&mut OrderState) -> T) -> T {
    let mut guard = state_cell().lock().expect("order state lock poisoned");
    f(&mut *guard)
}
...
impl OrderFulfillmentActor {
    ...
    fn workflow_run(&mut self, _from_actor: &str, payload: &[u8]) -> Result<Vec<u8>, String> {
        let payload = parse_payload(payload)?;
        Ok(json_bytes(
            serde_json::from_str(&run_workflow(&payload)).unwrap_or_else(
    ...
    fn workflow_query_status(
        ...
        Ok(json_bytes(
            serde_json::from_str(&query_status()).unwrap_or_else(
...
impl Guest for OrderFulfillmentBridge {
    ...
    fn set_state(state: Vec<u8>) -> Result<(), String> {
        ...
        match serde_json::from_slice::<OrderState>(&state) {
            Ok(next) => {
                with_state(|state| {
                    *state = next;
                });
...
mod tests {
    ...
    fn parse_op_reads_embedded_op() {
        assert_eq!(
            parse_op("call", br#"{"op":"workflow_run","order_id":"a"}"#).expect("op"),
            "workflow_run"
        );
    }
    ...
    fn actor_application_id_parses_qualified_actor() {
        assert_eq!(
            actor_application_id(
                "01ABC//order-fulfillment::temporal-order-fulfillment-rust@test-node"
            ),
