...
OUTPUT_WASM="$SCRIPT_DIR/web_crawl_actor.wasm"
EMBEDDED_WASM="$SCRIPT_DIR/web_crawl_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/web_crawl_wasm.wasm"
...
echo "Building Web Crawl (Rust WASM)..."
...
for candidate in \
  ...
  "$REPO_ROOT/examples/typescript/apps/abstractions/node_modules/@bytecodealliance/jco/lib/wasi_snapshot_preview1.reactor.wasm"; do
  if [ -f "$candidate" ]; then
    ADAPTER="$candidate"
    break
  fi
