#!/bin/bash
# Build Payment Workflow WASM component (Go/TinyGo) - Cadence-style
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
WIT_DIR="$REPO_ROOT/wit/plexspaces-actor"
OUTPUT_WASM="$SCRIPT_DIR/payment_workflow_actor.wasm"

cd "$SCRIPT_DIR"

echo "Building payment workflow (Go - Cadence-style)..."
...
rm -f payment_workflow_core.wasm payment_workflow_embedded.wasm
