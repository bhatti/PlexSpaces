#!/usr/bin/env bash
# Build Exactly-Once Payment WASM actor (TypeScript) - Restate style
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
WIT_DIR="$REPO_ROOT/wit/plexspaces-actor"
OUTPUT_WASM="$SCRIPT_DIR/payment_actor.wasm"

cd "$SCRIPT_DIR"

echo "Building TypeScript payment (Restate-style, exactly-once) → WASM..."
echo ""
...
if [ ! -f "payment_actor.js" ]; then
  echo "  ERROR: payment_actor.js not produced"
  exit 1
fi
echo "  ✓ payment_actor.js"
...
if ! $JCO componentize payment_actor_bundle.mjs --wit "$WIT_DIR" -n actor-world -o "$OUTPUT_WASM" --disable all 2>&1; then
