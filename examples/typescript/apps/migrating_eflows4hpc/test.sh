#!/bin/bash
# Test HPC Ensemble - EFlows4HPC style (TypeScript WASM)
...
APP_ID="migrating-eflows4hpc-ensemble-ts"
ACTOR_TYPE="ensemble"
NUM_TASKS=20
...
echo "================================================================"
echo "  EFlows4HPC → PlexSpaces: HPC Ensemble (TypeScript WASM)"
...
DEPLOY_OUT=$(curl -s -w "\n%{http_code}" -X POST "http://localhost:$HTTP_PORT/api/v1/applications/deploy" \
  ...
  -F "name=migrating-eflows4hpc-ensemble-ts" \
  -F "version=1.0.0" \
