...
// - ActorFactory.spawn_actor() for BehaviorRegistry-based actors
...
use crate::general::{General, GeneralMessage, Value, Decision};
...
impl Application for ByzantineApplication {
    ...
    async fn start(&mut self, node: Arc<dyn ApplicationNode>) -> Result<(), ApplicationError> {
        ...
        let mut metrics_tracker = CoordinationComputeTracker::new("byzantine-consensus".to_string());
        ...
        info!("  Behavior registration: {:.2}ms", register_time.as_secs_f64() * 1000.0);
        println!();
        ...
        metrics_tracker.end_coordinate();
        info!("  Spawned {} generals in {:.2}ms", self.config.general_count, spawn_time.as_secs_f64() * 1000.0);
        println!();
        ...
        for (i, general_ref) in general_refs.iter().enumerate() {
            ...
            general_ref.tell(init_msg).await
                .map_err(|e| ApplicationError::StartupFailed(
                    format!("Failed to initialize general {}: {}", i, e)
                ))?;
            metrics_tracker.increment_message();
        }
        ...
        metrics_tracker.end_coordinate();
        info!("  Initialized {} generals in {:.2}ms", self.config.general_count, init_time.as_secs_f64() * 1000.0);
        println!();

        // =====================================================================
        // Step 4: Run consensus protocol with message passing
        // =====================================================================
        info!("Running consensus protocol ({} rounds)...", self.config.num_rounds);
        info!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        ...
        for round in 0..self.config.num_rounds {
            ...
            let source_value = if round % 2 == 0 { Value::Zero } else { Value::One };
            ...
            for general_ref in &general_refs {
                general_ref.tell(source_msg.clone()).await
                    .map_err(|e| ApplicationError::StartupFailed(
                        format!("Failed to send vote: {}", e)
                    ))?;
                metrics_tracker.increment_message();
            }
            ...
            for (i, general_ref) in general_refs.iter().enumerate() {
                ...
                for (j, target_ref) in general_refs.iter().enumerate() {
                    if i != j {
                        ...
                        target_ref.tell(relay_msg).await
                            .map_err(|e| ApplicationError::StartupFailed(
                                format!("Failed to relay vote: {}", e)
                            ))?;
                        metrics_tracker.increment_message();
                    }
                }
            ...
            for (i, general_ref) in general_refs.iter().enumerate() {
                ...
                let reply = general_ref.ask(decision_msg, std::time::Duration::from_secs(5)).await
                    .map_err(|e| ApplicationError::StartupFailed(
                        format!("Failed to get decision from general {}: {}", i, e)
                    ))?;
                
                let decision: Decision = serde_json::from_slice(&reply.payload)
                    .map_err(|e| ApplicationError::StartupFailed(
                        format!("Failed to deserialize decision: {}", e)
                    ))?;
            ...
            // Count consensus
            let zero_count = round_decisions.values()
                .filter(|d| d.value == Value::Zero && !d.is_faulty)
                .count();
            let one_count = round_decisions.values()
                .filter(|d| d.value == Value::One && !d.is_faulty)
                .count();
            
            info!("  Round {} complete: {:.2}ms (compute: {:.2}ms)", 
                round + 1, round_time.as_secs_f64() * 1000.0, compute_time.as_secs_f64() * 1000.0);
            info!("  Decisions: {} Zero, {} One (honest generals)", zero_count, one_count);
        }
        ...
        for decisions in &consensus_results {
            let zero_count = decisions.values()
                .filter(|d| d.value == Value::Zero && !d.is_faulty)
                .count();
            let one_count = decisions.values()
                .filter(|d| d.value == Value::One && !d.is_faulty)
                .count();
        ...
        println!("  Honest: {}", self.config.general_count - self.config.fault_count);
        ...
        println!("{}", "─".repeat(80));
        println!("  Behavior registration: {:>12.2} ms (coordination)", register_time.as_secs_f64() * 1000.0);
        println!("  Actor spawning:        {:>12.2} ms (coordination)", spawn_time.as_secs_f64() * 1000.0);
        println!("  Initialization:        {:>12.2} ms (coordination)", init_time.as_secs_f64() * 1000.0);
        println!("  Message passing:       {:>12.2} ms (coordination)", metrics.coordinate_duration_ms as f64 - register_time.as_secs_f64() * 1000.0 - spawn_time.as_secs_f64() * 1000.0 - init_time.as_secs_f64() * 1000.0);
        println!("  Decision computation:  {:>12.2} ms (computation)", metrics.compute_duration_ms as f64);
        println!("  {}", "─".repeat(30));
        println!("  Coordination: {:>10.2} ms (total)", metrics.coordinate_duration_ms as f64);
        println!("  Computation:  {:>10.2} ms (total)", metrics.compute_duration_ms as f64);
        println!("  Total Time:   {:>10.2} ms ({:.2} seconds)", 
            total_time.as_secs_f64() * 1000.0, total_time_secs);
        
        println!("\n{}", "─".repeat(80));
        println!("📈 COORDINATION vs COMPUTATION ANALYSIS");
        println!("{}", "─".repeat(80));
        println!("  Computation time:      {:>12.2} ms", metrics.compute_duration_ms as f64);
        println!("  Coordination time:    {:>12.2} ms", metrics.coordinate_duration_ms as f64);
        println!("  Granularity ratio:     {:>12.2}× (compute/coordinate)", metrics.granularity_ratio);
        println!("  Efficiency:            {:>12.2}% (compute/total)", metrics.efficiency * 100.0);
        ...
        println!("\n  Cost Breakdown:");
        println!("    Coordination overhead: {:>8.2}% of total time", coord_cost_pct);
        println!("    Computation:           {:>8.2}% of total time", compute_cost_pct);
        
        println!("\n{}", "─".repeat(80));
        println!("🚀 BENCHMARK METRICS");
        println!("{}", "─".repeat(80));
        println!("  Messages/sec:          {:>12.2} msg/s", messages_per_sec);
        println!("  Consensus success:     {:>12.2}% ({} of {} rounds)", 
            consensus_rate, consensus_reached, self.config.num_rounds);
        println!("  Avg messages/round:    {:>12.2}", total_messages as f64 / self.config.num_rounds as f64);
        ...
        if metrics.granularity_ratio < 10.0 {
            ...
            println!("     - Current ratio: {:.2}× (should be >= 10×)", metrics.granularity_ratio);
        } else if metrics.granularity_ratio < 100.0 {
            println!("  ✓  ACCEPTABLE: Reasonable granularity for this problem size");
            println!("     - Ratio: {:.2}× (good for consensus algorithms)", metrics.granularity_ratio);
        } else {
            println!("  ✓  EXCELLENT: Good compute/coordinate ratio");
            println!("     - Ratio: {:.2}× (ideal for parallel efficiency)", metrics.granularity_ratio);
        }
        
        if coord_cost_pct > 20.0 {
            println!("  ⚠️  Coordination overhead is {:.1}% - consider more computation per round", coord_cost_pct);
        } else {
            println!("  ✓  Coordination overhead is {:.1}% - acceptable", coord_cost_pct);
        }
