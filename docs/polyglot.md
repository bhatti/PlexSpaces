...
export class CalculatorActor extends PlexSpacesActor<CalculatorState> {
  getDefaultState(): CalculatorState {
    return { lastResult: 0 };
  }

  onCalculate(payload: Record<string, unknown>) {
...
const instance = new CalculatorActor();
export const actor = {
...
**Note**: The SDK handles WIT interface details, iterative JSON serialization, and all boilerplate. Client code only needs to extend `PlexSpacesActor` and implement handlers.
...
```bash
# Install dependencies (includes @plexspaces/sdk)
npm install
