...
use plexspaces_core::{
    Actor as ActorTrait, ActorContext, ActorId, BehaviorError, BehaviorType, Message, ServiceLocator,
};
...
async fn register_counter_behavior(node: &plexspaces_node::Node, actor_type: &str) {
    ...
    registry.register(&t, |args: &[u8]| {
        let payload = args.to_vec();
        Box::pin(async move { Ok(Box::new(CounterActor::from_init_payload(&payload)) as Box<dyn plexspaces_core::Actor>) })
    }).await;
    node.service_locator().register_behavior_registry(Arc::new(registry)).await;
}

fn virtual_facet() -> Box<VirtualActorFacet> {
    Box::new(VirtualActorFacet::new(serde_json::json!({ "activation_strategy": "lazy" }), 100))
}
...
fn named_def_spec(name: &str, actor_type: &str, initial_count: i64) -> ActorSpawnSpec {
    ActorSpawnSpec {
        identity: Some(ActorIdentity { name: name.to_string(), actor_type: actor_type.to_string() }),
        role: String::new(),
        ...
        facets: vec![Facet {
            ...
            priority: 0, state: HashMap::new(), metadata: None,
        }],
        labels: HashMap::new(),
        config: None,
    }
...
async fn test_non_durable_named_actor_reactivates_with_definition_args() {
    ...
    let node = Arc::new(NodeBuilder::new(node_id).with_in_memory_backends().build().await);
    let actor_type = "abstractions_wasm";

    register_counter_behavior(&node, actor_type).await;

    let manager = node.service_locator().virtual_actor_manager().await.unwrap();

    // Register named definition: "ephemeral" → actor_type="abstractions_wasm", initial_count=5
    manager.register_virtual_actor_definition(named_def_spec("ephemeral", actor_type, 5)).await.unwrap();

    // Spawn instance "session-1"
    let actor_id = ActorId::new("session-1", actor_type, NAMESPACE, node_id).unwrap();
    let def_meta = manager.get_virtual_actor_definition(NAMESPACE, "ephemeral").await.unwrap();
    ...
    let status = registry_ask(&node, &actor_id, call_msg(serde_json::to_vec(&Cmd::Status).unwrap()), ASK_TIMEOUT).await;
    assert!(status.is_ok(), "initial status should succeed: {:?}", status.err());
    let s: StatusReply = serde_json::from_slice(&status.unwrap().payload).unwrap();
    assert_eq!(s.count, 5, "initial count must be 5 from definition args, got {}", s.count);

    // Increment by 2 → count = 7
    registry_ask(&node, &actor_id, call_msg(serde_json::to_vec(&Cmd::Increment { amount: 2 }).unwrap()), ASK_TIMEOUT).await.expect("increment must succeed");
    sleep(Duration::from_millis(100)).await;

    let status = registry_ask(&node, &actor_id, call_msg(serde_json::to_vec(&Cmd::Status).unwrap()), ASK_TIMEOUT).await.unwrap();
    ...
    let stop_ctx = plexspaces_core::RequestContext::new_without_auth(TENANT.to_string(), NAMESPACE.to_string());
    factory.stop_actor(&stop_ctx, &actor_id).await.unwrap();
    sleep(Duration::from_millis(200)).await;

    // is_virtual must still be true so routing can trigger reactivation
    assert!(manager.is_virtual(&actor_id).await, "actor must still be virtual after stop");
    ...
    let _ = registry_ask(&node, &actor_id, call_msg(serde_json::to_vec(&Cmd::Status).unwrap()), ASK_TIMEOUT).await;
    sleep(Duration::from_millis(300)).await;

    // Poll until count=5 (mirrors test.sh wait_for_json_field pattern)
    let deadline = std::time::Instant::now() + Duration::from_secs(5);
    loop {
        let reply = registry_ask(&node, &actor_id, call_msg(serde_json::to_vec(&Cmd::Status).unwrap()), ASK_TIMEOUT).await;
        if let Ok(r) = reply {
...
async fn test_named_definition_namespace_isolation() {
    ...
    let def_spec = ActorSpawnSpec {
        identity: Some(ActorIdentity { name: "worker".to_string(), actor_type: actor_type.to_string() }),
        role: String::new(),
    ...
    manager.register_virtual_actor_definition(def_spec).await.unwrap();

    let in_a = ActorId::new("w1", actor_type, "ns-a", node_id).unwrap();
    assert!(manager.is_virtual(&in_a).await, "actor in ns-a must be virtual");

    let in_b = ActorId::new("w1", actor_type, "ns-b", node_id).unwrap();
    assert!(!manager.is_virtual(&in_b).await, "same actor_type in ns-b must NOT be virtual");
}
...
async fn test_durable_actor_instance_retained_after_stop() {
    ...
    let manager = node.service_locator().virtual_actor_manager().await.unwrap();
    let def_spec = ActorSpawnSpec {
        identity: Some(ActorIdentity { name: "cart".to_string(), actor_type: actor_type.to_string() }),
        role: String::new(),
        namespace: NAMESPACE.to_string(),
        tenant_id: TENANT.to_string(),
        behavior_kind: "GenServer".to_string(),
    ...
    let def_meta = manager.get_virtual_actor_definition(NAMESPACE, "cart").await.unwrap();
    manager.prime_instance_from_definition(&actor_id, &def_meta).await;
    ...
    let factory = node.service_locator().get_actor_factory().await.unwrap();
    let req_ctx = plexspaces_core::RequestContext::new_without_auth(TENANT.to_string(), NAMESPACE.to_string());
