...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create workflow actor with durability
    let behavior = Box::new(OrderWorkflow::new(order.clone()));
    let mut actor = ActorBuilder::new(behavior)
        .with_id(format!("order-workflow-{}@comparison-node-1", order.id))
        .build()
        .await;
    
    // Attach DurabilityFacet (Temporal-style durable workflows with journaling)
    ...
    let workflow_actor = plexspaces_actor::ActorRef::local(
        core_ref.id().clone(),
        mailbox,
...
mod tests {
    ...
    async fn test_order_workflow() {
        ...
        let actor = ActorBuilder::new(behavior)
            .with_id("test-workflow@test-node".to_string())
            .build()
            .await;
