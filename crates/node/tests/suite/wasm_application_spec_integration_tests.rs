...
async fn test_wasm_deployment_creates_applicationspec() {
    ...
    assert!(
        app_state.is_some(),
        "Application should be registered with name 'test-application'"
    );
...
async fn test_wasm_deployment_dashboard_reflects_changes() {
    ...
    let before_response = DashboardService::get_summary(&dashboard_service, before_request)
        .await
        .unwrap();
    ...
    assert!(
        ...
        "Deployment should succeed. Status: {}",
    ...
    let after_response = DashboardService::get_summary(&dashboard_service, after_request)
        .await
        .unwrap();
    ...
    assert_eq!(
        after_app_count, 1,
        "Should show 1 application after deployment"
    );
    ...
    let apps_response = dashboard_service
        .get_applications(apps_request)
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        1,
        "Should have 1 application in list"
    );
    assert_eq!(
        apps.applications[0].name, "dashboard-test",
...
async fn test_wasm_deployment_undeployment_flow() {
    ...
    let summary_response = DashboardService::get_summary(&dashboard_service, summary_request)
        .await
        .unwrap();
...
async fn test_wasm_deployment_multiple_applications() {
    ...
    let summary = summary_response.into_inner();
    assert_eq!(
        summary.total_applications, 3,
        "Dashboard should show 3 applications"
    );
...
async fn test_wasm_deployment_component_error_handling() {
    ...
    // Try to use calculator WASM (component) if it exists (use shared module for performance)
    ...
    if status.is_success() {
        ...
        assert!(
            response_text.contains("component") || response_text.contains("Component"),
...
async fn test_wasm_deployment_applicationspec_auto_generation() {
    ...
    assert!(
        app_state.is_some(),
        "Application should be registered (ApplicationSpec auto-generated)"
    );
    assert_eq!(
        app_state,
        Some(ApplicationState::ApplicationStateRunning),
...
async fn test_wasm_deployment_complete_workflow() {
    ...
    let final_response = DashboardService::get_summary(&dashboard_service, final_request)
        .await
        .unwrap();
