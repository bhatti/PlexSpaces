...
`.trim();
class CoordinatorActor extends PlexSpacesActor {
    getDefaultState() {
    ...
    onResult(payload) {
        ...
        if (actorId) {
            ...
            if (next) {
                ...
                if (this.state.workers[actorId])
                    this.state.workers[actorId].current_p = undefined;
                try {
    ...
    onStatus() {
        ...
        const worker_count = Math.max(Object.keys(this.state.workers).length, pg_members.length);
        // Per-worker details
    ...
    _nextPending(cpuCores) {
        const pending = Object.values(this.state.work)
...
// ─── CodeServerActor ──────────────────────────────────────────────────────────
class CodeServerActor extends PlexSpacesActor {
