...
echo -e "${BLUE}=== Finance Risk Assessment Cleanup ===${NC}"
echo ""

cd "$PROJECT_DIR"

# Check if cluster is running
if [ -f "/tmp/finance-risk/coordinator.pid" ]; then
    if ps -p $(cat /tmp/finance-risk/coordinator.pid) > /dev/null 2>&1; then
...
echo "  - SQLite journal databases (data/finance/)"
echo "  - Log files (logs/)"
echo "  - PID files (/tmp/finance-risk/)"
...
if [ -d "/tmp/finance-risk" ]; then
    echo -e "${BLUE}Removing PID files...${NC}"
    rm -rf /tmp/finance-risk
