...
- **NodeService**: Node management, metrics, and capacity (`crates/services/src/node_service/`). Implements `NodeConnectivity` for seed-node connection (cluster_seed_nodes at startup, ApplicationSpec.seed_nodes on deploy); see [Node connectivity and seed nodes](../../docs/services.md#node-connectivity-and-seed-nodes).
