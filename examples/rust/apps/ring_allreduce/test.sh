...
WASM_FILE="$SCRIPT_DIR/ring_allreduce_actor.wasm"
...
APP_ID="ring-allreduce-rust"
APP_NAME="ring-allreduce-rust"
LEADER_ACTOR="leader"
VECTOR_SIZE=16384
WORKER_COUNT=16
ROUNDS=24
...
TEMP_CONFIG="$(mktemp -t ring-allreduce-app-config)"
...
run_payload=$(cat <<EOF
{"op":"run","vector_size":$VECTOR_SIZE,"worker_count":$WORKER_COUNT,"rounds":$ROUNDS,"values_per_shard":$VALUES_PER_SHARD}
EOF
)
...
echo "Step 3: Metrics"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
export RING_ALLREDUCE_WALL_MS="$wall_ms"
export RING_ALLREDUCE_EXPECTED_NODES="${#NODE_LIST[@]}"
...
wall_ms = int(os.environ.get("RING_ALLREDUCE_WALL_MS", "0"))
...
reduce_ops = int(payload.get("reduce_operation_count", 0))
values_reduced = int(payload.get("values_reduced", 0))
error_count = int(payload.get("error_count", 0))
nodes = payload.get("nodes", {})
node_addresses = payload.get("node_addresses", {})
roles = payload.get("roles", {})
remote_nodes_with_work = payload.get("remote_nodes_with_work", [])
expected_nodes = int(os.environ.get("RING_ALLREDUCE_EXPECTED_NODES", "1"))

print("  Ring AllReduce (Rust WASM)")
print("  Data size")
print(
    "  vector_size={vector} worker_count={workers} rounds={rounds} values_per_shard={values_per_shard}".format(
        vector=payload.get("vector_size", 0),
        workers=payload.get("worker_count", 0),
        rounds=payload.get("rounds", 0),
        values_per_shard=payload.get("values_per_shard", 0),
    )
)
print("  Topology")
print(
    "  node_count={node_count} worker_node_count={worker_nodes} actor_count={actor_count} leader_node_id={leader} ring_steps={steps}".format(
        node_count=node_count,
        worker_nodes=worker_node_count,
        actor_count=actor_count,
        leader=payload.get("leader_node_id", ""),
        steps=payload.get("ring_steps", 0),
    )
)
print("  Timing")
...
print(f"  reduce_operation_count={reduce_ops}")
print(f"  values_reduced={values_reduced}")
print("  Result")
print(f"  checksum={payload.get('reduced_checksum', 0)}")
...
for role in sorted(roles):
    info = roles[role]
    print(
        "  {role}: actors={actors} messages={messages} reduce_ops={reduce_ops} values={values} compute_ms={compute_ms} coord_ms={coord_ms} avg_latency_ms={avg:.2f} max_latency_ms={max_latency} errors={errors}".format(
            role=role,
            actors=info.get("actors", 0),
            messages=info.get("messages", 0),
            reduce_ops=info.get("reduce_operations", 0),
            values=info.get("values_reduced", 0),
...
for node_id in sorted(nodes):
    ...
    print(
        "  {node_id} ({address}): actors={actors} leader_actors={leader_actors} worker_actors={worker_actors} messages={messages} leader_messages={leader_messages} worker_messages={worker_messages} reduce_ops={reduce_ops} values={values} compute_ms={compute_ms} coord_ms={coord_ms} avg_latency_ms={avg:.2f} max_latency_ms={max_latency} errors={errors}".format(
            node_id=node_id,
            address=address,
            actors=info.get("actors", 0),
...
if reduce_ops <= 0:
    raise SystemExit("expected positive reduce operation count")
if values_reduced <= 0:
    raise SystemExit("expected positive values_reduced count")
PY
  echo -e "${RED}Metrics validation failed${NC}"
  exit 1
}
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo -e "${GREEN}Ring allreduce test passed.${NC}"
