...
use plexspaces_core::{ReplyWaiter, ReplyWaiterError, ReplyWaiterRegistry};
...
async fn test_reply_waiter_concurrent_waiters() {
    ...
    let waiter_clone = waiter.clone();
    let handle = tokio::spawn(async move { waiter_clone.wait(Duration::from_secs(1)).await });
    ...
    let handle1 = tokio::spawn(async move { waiter1_clone.wait(Duration::from_secs(1)).await });
    let handle2 = tokio::spawn(async move { waiter2_clone.wait(Duration::from_secs(1)).await });
