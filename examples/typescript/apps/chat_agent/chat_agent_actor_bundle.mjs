...
// chat_agent_actor.ts
var _ChatAgentActor = class _ChatAgentActor extends PlexSpacesActor {
  getDefaultState() {
    return {
      agent_id: "",
      total_messages: 0,
  ...
  onChat(payload) {
    ...
    try {
      ...
      const resp = host.httpClient("llm-link").post(
        "/v1/messages",
        JSON.parse(llmBody)
      );
      const body = JSON.parse(resp.body);
      assistantReply = body?.content?.[0]?.text ?? body?.choices?.[0]?.message?.content ?? "[no response]";
    } catch (e) {
    ...
    if (history.length > _ChatAgentActor.ALARM_THRESHOLD) {
      const alarmAt = host.alarm.get();
      if (alarmAt === 0) {
        host.alarm.set(host.nowMs() + _ChatAgentActor.ALARM_DELAY_MS);
  ...
  on__alarm__(_payload) {
    ...
    try {
      ...
      const body = JSON.parse(resp.body);
      summary = body?.content?.[0]?.text ?? body?.choices?.[0]?.message?.content ?? "[summary unavailable]";
    } catch {
...
_ChatAgentActor.ALARM_THRESHOLD = 10;
// Summarization alarm delay: 5 minutes in milliseconds.
_ChatAgentActor.ALARM_DELAY_MS = 3e5;
var ChatAgentActor = _ChatAgentActor;
var _actor = new ChatAgentActor();
var actor2 = {
