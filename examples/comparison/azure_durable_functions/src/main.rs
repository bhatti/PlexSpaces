...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create DurabilityFacet (Azure Durable Functions-style durable execution)
    ...
    let actor_id = format!("order-orchestration-{}@comparison-node-1", order.order_id);
    ...
    let orchestration = plexspaces_actor::ActorRef::remote(
        actor_id.clone(),
...
mod tests {
    ...
    async fn test_order_orchestration() {
        ...
        let actor_id = "test-orchestration@test-node".to_string();
        let ctx = plexspaces_core::RequestContext::internal();
        let _message_sender = actor_factory.spawn_actor(
            &ctx,
            &actor_id,
