...
EMBEDDED_WASM="$SCRIPT_DIR/scheduler_actor_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/migrating_skypilot_wasm.wasm"
...
echo "Building SkyPilot scheduler (Rust WASM, SDK+WIT)..."
