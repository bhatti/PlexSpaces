...
OUTPUT_WASM="$SCRIPT_DIR/entity_recognition_actor.wasm"
EMBEDDED_WASM="$SCRIPT_DIR/entity_recognition_actor_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/entity_recognition_wasm.wasm"
...
echo "Building entity recognition (Rust WASM, SDK+WIT)..."
