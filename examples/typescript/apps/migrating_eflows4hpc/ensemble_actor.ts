...
export class EnsembleActor extends WorkflowActor<EnsembleState> {
  ...
  run(payload: Record<string, unknown>): Record<string, unknown> {
    ...
    const pattern: unknown[] = [RESULT_PREFIX, this.state.ensemble_id, "result", null, null];
  ...
  onTasks_ready(payload: Record<string, unknown>): Record<string, unknown> {
    ...
    const t0 = host.nowMs();
    const pattern: unknown[] = [RESULT_PREFIX, ensembleId, "task", null, null];
    let processed = 0;
    let computeMs = 0;
    while (true) {
