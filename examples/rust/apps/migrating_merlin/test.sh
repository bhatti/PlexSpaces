#!/bin/bash
# Test parameter sweep - Merlin style (Rust WASM), worker pool with elastic size
# Uses pool API when configured; fallback: process group. sweep:coord-1 = coordinator; sweep:worker-0,1,... = workers.
...
echo "  Pool API + tuple space; fallback: process group (Rust WASM)"
echo "================================================================"
...
for i in $(seq 1 $BATCH_SWEEPS); do
  send_op "coord-1" "{\"op\":\"workflow_run\",\"sweep_id\":\"batch-$i\",\"num_params\":$BATCH_PARAMS_PER_SWEEP}" 30 >/dev/null
done
BATCH_END=$(date +%s%N)
