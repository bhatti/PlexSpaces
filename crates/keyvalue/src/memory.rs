...
impl KeyValueStore for InMemoryKVStore {
    ...
    async fn put(&self, key: &str, value: Vec<u8>) -> KVResult<()> {
        let mut data = self.data.write().await;
        let old_value = data.get(key).map(|e| e.value.clone());
        data.insert(key.to_string(), Entry::new(value.clone()));
    ...
    async fn delete(&self, key: &str) -> KVResult<()> {
        let mut data = self.data.write().await;
        let old_value = data.remove(key).map(|e| e.value);
    ...
    async fn put_with_ttl(&self, key: &str, value: Vec<u8>, ttl: Duration) -> KVResult<()> {
        let mut data = self.data.write().await;
        let old_value = data.get(key).map(|e| e.value.clone());
        data.insert(key.to_string(), Entry::new_with_ttl(value.clone(), ttl));
    ...
    async fn cas(
        ...
        let current = data.get(key).filter(|e| !e.is_expired()).map(|e| &e.value);
        ...
        if matches {
            let old_value = data.get(key).map(|e| e.value.clone());
            data.insert(key.to_string(), Entry::new(new_value.clone()));
    ...
    async fn increment(&self, key: &str, delta: i64) -> KVResult<i64> {
        ...
        data.insert(key.to_string(), Entry::new(encoded.clone()));
    ...
    async fn stats(&self) -> KVResult<KVStats> {
        let data = self.data.read().await;
        let total_keys = data.iter().filter(|(_, e)| !e.is_expired()).count();
...
mod tests {
    ...
    async fn test_multi_get() {
        ...
        let values = kv.multi_get(&["k1", "k2", "k3"]).await.unwrap();
    ...
    async fn test_multi_put() {
        ...
        kv.multi_put(&[("k1", b"v1".to_vec()), ("k2", b"v2".to_vec())])
            .await
            .unwrap();

        assert_eq!(kv.get("k1").await.unwrap(), Some(b"v1".to_vec()));
        assert_eq!(kv.get("k2").await.unwrap(), Some(b"v2".to_vec()));
    }
    ...
    async fn test_refresh_ttl() {
        ...
        // Refresh TTL
        kv.refresh_ttl("key", Duration::from_secs(2)).await.unwrap();
    ...
    async fn test_cas() {
        ...
        let released = kv
            .cas("lock:resource", Some(b"node1".to_vec()), b"node2".to_vec())
            .await
            .unwrap();
        assert!(released);
    }
    ...
    async fn test_watch() {
        let kv = InMemoryKVStore::new();
        let mut watcher = kv.watch("config:timeout").await.unwrap();

        // Put value
        kv.put("config:timeout", b"30s".to_vec()).await.unwrap();
    ...
    async fn test_watch_prefix() {
        let kv = InMemoryKVStore::new();
        let mut watcher = kv.watch_prefix("config:actor.").await.unwrap();

        // Put value
        kv.put("config:actor.timeout", b"30s".to_vec())
            .await
            .unwrap();
    ...
    async fn test_clear_prefix() {
        ...
        kv.put("temp:1", b"a".to_vec()).await.unwrap();
        kv.put("temp:2", b"b".to_vec()).await.unwrap();
        kv.put("keep:1", b"c".to_vec()).await.unwrap();

        let deleted = kv.clear_prefix("temp:").await.unwrap();
        assert_eq!(deleted, 2);

        assert!(!kv.exists("temp:1").await.unwrap());
        assert!(!kv.exists("temp:2").await.unwrap());
        assert!(kv.exists("keep:1").await.unwrap());
    }
    ...
    async fn test_count_prefix() {
        ...
        kv.put("actor:a", b"1".to_vec()).await.unwrap();
        kv.put("actor:b", b"2".to_vec()).await.unwrap();
        kv.put("node:n1", b"3".to_vec()).await.unwrap();

        let count = kv.count_prefix("actor:").await.unwrap();
        assert_eq!(count, 2);
    }
