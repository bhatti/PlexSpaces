...
# Test ChatAgent: validates state management and alarm logic.
...
echo "Step 2: Send a chat message (LLM may return placeholder if key missing)"
RESP=$(send_op "$AGENT_ACTOR" '{"op":"chat","message":"Hello, who are you?"}')
echo "  Response: $RESP"
check_ok "$RESP" "chat"

echo "Step 3: Get conversation history (must contain at least 1 message)"
...
echo "Step 4: Send another chat message"
