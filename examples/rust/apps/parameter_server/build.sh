...
OUTPUT_WASM="$SCRIPT_DIR/parameter_server_actor.wasm"
EMBEDDED_WASM="$SCRIPT_DIR/parameter_server_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/parameter_server_wasm.wasm"
...
echo "Building Parameter Server (Rust WASM)..."
