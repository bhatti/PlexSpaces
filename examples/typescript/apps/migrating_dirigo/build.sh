#!/usr/bin/env bash
# Build windowed stream actor WASM: TypeScript → JS → ESM bundle → WASM component
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
WIT_DIR="$REPO_ROOT/wit/plexspaces-actor"
OUTPUT_WASM="$SCRIPT_DIR/stream_actor.wasm"

cd "$SCRIPT_DIR"

echo "Building TypeScript stream (Dirigo windowed aggregation) → WASM..."
echo ""
...
if [ ! -f "stream_actor.js" ]; then
  echo "  ERROR: stream_actor.js not produced"
  exit 1
fi
echo "  ✓ stream_actor.js"
...
if ! $JCO componentize stream_actor_bundle.mjs --wit "$WIT_DIR" -n actor-world -o "$OUTPUT_WASM" --disable all 2>&1; then
