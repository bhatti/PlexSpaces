...
OUTPUT_WASM="$SCRIPT_DIR/agentic_rag_pipeline_actor.wasm"

cd "$SCRIPT_DIR"

echo "Building Agentic RAG Pipeline (Go WASM)..."
...
tinygo build -target=wasi -o agentic_rag_pipeline_actor_core.wasm .
wasm-tools component embed "$WIT_DIR" -w actor-world agentic_rag_pipeline_actor_core.wasm -o agentic_rag_pipeline_actor_embedded.wasm
wasm-tools component new agentic_rag_pipeline_actor_embedded.wasm --adapt "wasi_snapshot_preview1=$ADAPTER" -o "$OUTPUT_WASM"
rm -f agentic_rag_pipeline_actor_core.wasm agentic_rag_pipeline_actor_embedded.wasm
