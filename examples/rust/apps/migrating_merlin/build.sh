#!/usr/bin/env bash
# Build Merlin parameter sweep WASM component (Rust): core wasm → component embed → WASI adapter
set -euo pipefail
...
EMBEDDED_WASM="$SCRIPT_DIR/sweep_actor_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/migrating_merlin_wasm.wasm"
...
echo "Building Merlin parameter sweep (Rust WASM, SDK+WIT)..."
