...
impl MessageSender for VirtualActorWrapper {
    async fn tell(&self, message: Message) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        eprintln!("🔵 [VIRTUAL_ACTOR_WRAPPER] tell() called: actor_id={}, message_id={}, message_type={:?}", 
        ...
        eprintln!("🔵 [VIRTUAL_ACTOR_WRAPPER] is_active check: actor_id={}, is_active={}, message_id={}", self.actor_id, is_active, message_id);
        if !is_active {
            // Actor is not activated - activate it
            eprintln!("🔵 [VIRTUAL_ACTOR_WRAPPER] Actor not activated, activating now: actor_id={}, message_id={}", self.actor_id, message_id);
            ...
            eprintln!("🟢 [VIRTUAL_ACTOR_WRAPPER] Message queued: actor_id={}, message_id={}", self.actor_id, message_id);
            ...
            eprintln!("🔵 [VIRTUAL_ACTOR_WRAPPER] Calling activate_virtual_actor: actor_id={}, message_id={}", self.actor_id, message_id);
            factory.activate_virtual_actor(&self.actor_id).await
                .map_err(|e| {
                    eprintln!("🔴 [VIRTUAL_ACTOR_WRAPPER] Failed to activate virtual actor: actor_id={}, message_id={}, error={}", self.actor_id, message_id, e);
                    format!("Failed to activate virtual actor: {}", e)
                })?;
            eprintln!("🟢 [VIRTUAL_ACTOR_WRAPPER] Actor activated: actor_id={}, message_id={}", self.actor_id, message_id);
            ...
            let sender = registry.lookup_actor(&self.actor_id).await
                .ok_or_else(|| {
                    eprintln!("🔴 [VIRTUAL_ACTOR_WRAPPER] Actor not found in registry after activation: actor_id={}", self.actor_id);
                    format!("Actor not found after activation: {}", self.actor_id)
                })?;
            
            eprintln!("🟢 [VIRTUAL_ACTOR_WRAPPER] Got ActorRef after activation, sending queued message: actor_id={}, message_id={}", self.actor_id, message_id);
            ...
            eprintln!("🔵 [VIRTUAL_ACTOR_WRAPPER] Retrieved {} queued messages after activation: actor_id={}", queued_messages.len(), self.actor_id);
            ...
            eprintln!("🟢 [VIRTUAL_ACTOR_WRAPPER] All queued messages sent after activation: actor_id={}, message_id={}", self.actor_id, message_id);
            return Ok(());
        }
        
        // Actor is activated - use MessageSender from registry
        // Get MessageSender (which will be ActorRef for activated actors)
        eprintln!("🔵 [VIRTUAL_ACTOR_WRAPPER] Actor is active, using ActorRef: actor_id={}", self.actor_id);
        use plexspaces_core::ActorRegistry;
        let registry: Arc<ActorRegistry> = self.service_locator.get_service_by_name(plexspaces_core::service_locator::service_names::ACTOR_REGISTRY).await
            .ok_or_else(|| "ActorRegistry not registered in ServiceLocator".to_string())?;
        ...
        let sender = registry.lookup_actor(&self.actor_id).await
            .ok_or_else(|| {
                eprintln!("🔴 [VIRTUAL_ACTOR_WRAPPER] Actor not found in registry: actor_id={}", self.actor_id);
                format!("Actor not found: {}", self.actor_id)
            })?;
        
        let message_id_for_send = message.id.clone();
        eprintln!("🔵 [VIRTUAL_ACTOR_WRAPPER] Sending message via ActorRef: actor_id={}, message_id={}", self.actor_id, message_id_for_send);
        sender.tell(message).await
            .map_err(|e| {
                eprintln!("🔴 [VIRTUAL_ACTOR_WRAPPER] MessageSender.tell() failed: actor_id={}, message_id={}, error={}", self.actor_id, message_id_for_send, e);
