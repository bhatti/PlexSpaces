...
EMBEDDED_WASM="$SCRIPT_DIR/chat_agent_actor_embedded.wasm"
BUILD_PROFILE="${CARGO_PROFILE:-debug}"
PROFILE_FLAG=""
if [ "$BUILD_PROFILE" = "release" ]; then
  PROFILE_FLAG="--release"
fi
CORE_WASM="$REPO_ROOT/target/wasm32-wasip1/$BUILD_PROFILE/chat_agent.wasm"
...
echo "Building ChatAgent (Rust WASM)..."
